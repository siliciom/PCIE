...
module PCIe_top;
     ...
     TX_TL_DL_Interface RC_TX_TL_DLL[NUM_RC] (
         CLK,
         RESET
     );
     TX_DLL_PCS_Interface RC_TX_DLL_PCS[NUM_RC] (
         CLK,
         RESET
     );
     pipe_tx_interface RC_TX_PIPE[NUM_RC] (
         RC_PCLK,
         RESET
     );
     ...
     RX_TL_DL_Interface EP_RX_TL_DLL[NUM_EP] (
         CLK,
         RESET
     );
     RX_DLL_PCS_Interface EP_RX_DLL_PCS[NUM_EP] (
         CLK,
         RESET
     );
     pipe_rx_interface EP_RX_PIPE[NUM_EP] (
         EP_PCLK,
         RESET
     );
     ...
     generate
          for (genvar i = 0; i < NUM_RC; i++) begin : g_rc_cfg
               ...
               initial begin
                    ...
                    uvm_config_db#(virtual TX_TL_DL_Interface)::set(
                        null, rc_name, "TL_Vif", RC_TX_TL_DLL[i]);  // TL interface
                    uvm_config_db#(virtual TX_DLL_PCS_Interface)::set(
                        null, rc_name, "DLL_Vif", RC_TX_DLL_PCS[i]);  // DL interface
                    uvm_config_db#(virtual pipe_tx_interface)::set(
                        null, rc_name, "pipe_Vif", RC_TX_PIPE[i]);  // Pipe interface
                    uvm_config_db#(virtual phy_tx_interface)::set(null, rc_name, "phy_Vif",
                                                                  RC_PHY_TX[i]);  // Phy interface
               end
     ...
     generate
          for (genvar i = 0; i < NUM_EP; i++) begin : g_ep_cfg
               ...
               initial begin
                    ...
                    uvm_config_db#(virtual RX_TL_DL_Interface)::set(
                        null, ep_name, "TL_Vif", EP_RX_TL_DLL[i]);  // TL interface
                    uvm_config_db#(virtual RX_DLL_PCS_Interface)::set(
                        null, ep_name, "DLL_Vif", EP_RX_DLL_PCS[i]);  //DL interface
                    uvm_config_db#(virtual pipe_rx_interface)::set(
                        null, ep_name, "pipe_Vif", EP_RX_PIPE[i]);  // Pipe interface
                    uvm_config_db#(virtual phy_rx_interface)::set(null, ep_name, "phy_Vif",
                                                                  EP_PHY_RX[i]);  // Phy interface
