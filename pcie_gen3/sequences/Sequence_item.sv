class Sequence_item extends uvm_sequence_item;
     ...
     constraint INJECT_ERR_DEFAULT {soft inject_err == ERR_NONE;}
               ...
               MEM_RD, MEM_WR: begin
                    ...
                    if (e_type == MEM_WR) begin
                         ...
                         foreach (payload[i]) begin
                              ...
                              printer.print_field($sformatf("payload[%0d]", i), payload[i], 32,
                                                  UVM_HEX);
     ...
     function void post_randomize();
          ...
          case (inject_err)
               ...
               ERR_EP_POISON: begin
                    if ((e_type == MEM_WR) || (e_type == MEM_RD)) ep = 1'b1;
               end
     ...
     constraint MEM_RD_CONSTRAINT {
          ...
          if (e_type == MEM_RD) {
               ...
               //length inside {[1:10]};
               payload.size() == 0;
               first_BE inside {4'b0000,4'b0001,4'b0011,4'b0111,4'b1111,4'b1000,4'b1100,4'b1110};
               last_BE  inside {4'b0000,4'b0001,4'b0011,4'b0111,4'b1111,4'b1000,4'b1100,4'b1110};
          }
     }
     ...
     constraint TRAFFIC_CLASS_CONSTRAINT {tc inside {[0 : 7]};}

     constraint ATTRIBUTE_1_CONSTRAINT {attr_1 inside {0, 1};}

     constraint TLP_PROCESSING_HINTS_CONSTRAINT {th inside {0, 1};}

     constraint TLP_DIGEST_CONSTRAINT {td inside {0, 1};}
     ...
     constraint ERROR_POISON_CONSTRAINT {ep == 0;}

     constraint ADDRESS_TRANSLATION_CONSTRAINT {at inside {2'b10};}

     constraint BUS_CONSTRAINT {bus inside {8'b0};}

     constraint DEVICE_CONSTRAINT {device inside {5'b1};}

     constraint FUNCTION_CONSTRAINT {function_n inside {3'b0};}

     constraint TAG_CONSTRAINT {tag inside {[0 : 255]};}


     constraint PKT_TYPE_CONSTRAINT {

          if (e_type inside {MEM_WR}) {pkt_type == P;}

          if (e_type inside {MEM_RD, IO_RD, IO_WR, CFG_RD0, CFG_RD1, CFG_WR0, CFG_WR1}) {
               pkt_type == NP;
          }

          if (e_type inside {CPL, CPL_DATA}) {pkt_type == CMPL;}
     ...
     function void pack_tlp();
          ...
          case (e_type)
               ...
               CFG_RD0, CFG_WR0, CFG_RD1, CFG_WR1: begin
                    `uvm_info("SEQUENCE_ITEM", $sformatf(
                              "EP_BUS = %0d, EP_DEVICE = %0d, EP_FUNCTION = %0d",
                              ep_bus,
                              ep_device,
                              ep_function_n
                              ), UVM_LOW)
                    ...
                    `uvm_info(
                        "PACK", $sformatf(
                        "PACK: reg_num=%0d ext_reg=%0d DW2=%08h", register_num, ext_register_num, dw
                        ), UVM_NONE)
                    tlp_q.push_back(dw);
          ...
          foreach (payload[i]) tlp_q.push_back(payload[i]);
          ...
          if (td) begin
               ...
               if (inject_err == ERR_ECRC) ecrc = ~ecrc;
