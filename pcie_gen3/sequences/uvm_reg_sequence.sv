...
class pcie_cfg_ro_check_seq extends pcie_ral_base_seq;
     ...
     task body();
          ...
          if (rdata !== expected)
               `uvm_error("RO_CHECK", $sformatf(
                          "vendor_device_id mismatch: got 0x%08h exp 0x%08h", rdata, expected))
          ...
          if (rdata !== expected)
               `uvm_error("RO_CHECK", $sformatf(
                          "classcode_revid mismatch: got 0x%08h exp 0x%08h", rdata, expected))
          ...
          if (rdata !== expected)
               `uvm_error("RO_CHECK", $sformatf(
                          "cap_pointer mismatch: got 0x%08h exp 0x%08h", rdata, expected))
          ...
          if (rdata !== expected)
               `uvm_error("RO_CHECK", $sformatf(
                          "subsystem_id mismatch: got 0x%08h exp 0x%08h", rdata, expected))
          ...
          if (rdata !== expected)
               `uvm_error("RO_CHECK", $sformatf(
                          "cardbus_cis mismatch: got 0x%08h exp 0x%08h", rdata, expected))

          `uvm_info("RO_CHECK", "Read-only register check complete", UVM_HIGH)
     endtask
endclass
...
class pcie_bar_sizing_seq extends pcie_ral_base_seq;
     ...
     task automatic size_one_bar(uvm_reg bar_reg, string bar_name);
          ...
          if (size_pattern == 0)
               `uvm_info("BAR_SIZE", $sformatf("%s: not implemented / size 0", bar_name),
                         UVM_LOW)  // Indicates bar size is not implemented
          else begin
               decoded_size = (~size_pattern) + 1;
               `uvm_info(
                   "BAR_SIZE", $sformatf(
                   "%s: aperture size = 0x%08h (%0d bytes)", bar_name, decoded_size, decoded_size),
                   UVM_LOW)
          end
...
class pcie_command_status_rw1c_seq extends pcie_ral_base_seq;
     ...
     task body();
          ...
          if (rdata[2] !== 1'b1 || rdata[1] !== 1'b1)
               `uvm_error("CMD_STATUS",
                          "bus_master_enable/memory_space_enable did not set as expected")
          ...
          if (rdata !== 1'b0)
               `uvm_error("CMD_STATUS",
                          "fast_b2b_capable (RO) changed via write — should be ignored")

          // W1C pattern: writing 1 clears the bit (uvm_reg_field configured
          // with access "W1C" auto-models this — no backdoor force needed).
          regmodel.command_status.detected_parity_error.write(status, 1'b1,
                                                              .map(regmodel.default_map));
          regmodel.command_status.detected_parity_error.read(status, rdata,
                                                             .map(regmodel.default_map));
          if (rdata !== 1'b0)
               `uvm_error("CMD_STATUS", "detected_parity_error did not clear after write-1 (W1C)")

          regmodel.command_status.signaled_target_abort.write(status, 1'b1,
                                                              .map(regmodel.default_map));
          regmodel.command_status.signaled_target_abort.read(status, rdata,
                                                             .map(regmodel.default_map));
          if (rdata !== 1'b0)
               `uvm_error("CMD_STATUS", "signaled_target_abort did not clear after write-1 (W1C)")

          `uvm_info("CMD_STATUS", "Command/Status register test complete", UVM_HIGH)
     endtask
endclass
...
class pcie_cfg_full_ral_seq extends pcie_ral_base_seq;
     ...
     task body();
          ...
          `uvm_info("PCIE_CFG_SEQ", "Full PCIe config-space RAL sequence complete", UVM_HIGH)
     endtask
endclass
