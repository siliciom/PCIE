...
class PCIe_MAC_driver extends uvm_driver #(Sequence_item);
     ...
     bit [31:0] rc_tlp_burst_q[$][$];  // one entry per already-STP-framed TLP, kept
                                       // can drive each TLP as its OWN PIPE burst
                                       // instead of concatenating whatever piled up.
     ...
     bit [31:0] ep_tlp_burst_q[$][$];  // EP-side mirror of rc_tlp_burst_q: one entry
                                       // per already-STP-framed TLP, kept separate
                                       // so ep_state_l0() drives each as its own burst.
     int ep_size;
     int ep_size_dll;
     int ep_size_tlp;
     ...
     typedef enum {
          ...
          CONFIG_LINKNUM_START,
          CONFIG_LINKNUM_ACCEPT,
          CONFIG_LANNUM_WAIT,
     ...
     typedef enum {
          ...
          EP_DETECT_ACTIVE,
          EP_POLLING_ACTIVE,
          EP_POLLING_COMPLIANCE,
          EP_POLLING_CONFIGURATION,
          EP_CONFIG_LINKNUM_START,
          EP_CONFIG_LINKNUM_ACCEPT,
          EP_CONFIG_LANNUM_WAIT,
          EP_CONFIG_LANENUM_ACCEPT,
          EP_CONFIG_COMPLETE,
          EP_CONFIG_IDLE,
          EP_L0,
          EP_RECOVERY_RCVRLOCK,
          EP_RECOVERY_RCVRCFG,
          EP_RECOVERY_SPEED,
          EP_RECOVERY_IDLE,
          EP_DISABLED,
     ...
     function void build_phase(uvm_phase phase);
          ...
          tag = get_full_name();
          `uvm_info("PCIe_MAC_driver", $sformatf("[%s] Configured: mode=%s", tag, cfg.mode.name()),
                    UVM_HIGH)
          ...
          case (cfg.mode)
               ...
               RC_MODE: begin
                    ...
                    `uvm_info("PCIe_MAC_driver", $sformatf("[%s] RC interfaces connected", tag),
                              UVM_HIGH)
               end
               ...
               EP_MODE: begin
                    ...
                    `uvm_info("PCIe_MAC_driver", $sformatf("[%s] EP interfaces connected", tag),
                              UVM_HIGH)
               end
     ...
     function void write_port_e(Sequence_item t);
          ...
          foreach (t.dllp_tx_packet[i])
               `uvm_info("MAC_TX_DRV", $sformatf(
                         "[%s] DLLP_INITIAL_TX %h %d DWORDs",
                         tag,
                         t.dllp_tx_packet[i],
                         t.dllp_tx_packet.size()
                         ), UVM_HIGH)
          ...
          // t.mac_tx_data. Previously these were then flattened into a single
          // rc_received queue, which meant rc_state_l0() would byte-stripe and
          // drive out whatever TLPs happened to have piled up as ONE continuous
          // PIPE burst (single TxDataValid assertion) - correctly framed with
          // per-TLP STPs internally, but not what's wanted when each TLP needs
          // to appear as its own separate burst on the wire. Instead, push each
          // framed TLP as its OWN entry into rc_tlp_burst_q (queue-of-queues) so
          // rc_state_l0() can pop and drive them one at a time, each getting its
          // own rc_byte_striping()/rc_drive_striped_data() call and therefore its
          // own TxDataValid assert/deassert (i.e. its own separate burst).
          foreach (t.mac_tx_data[i]) begin
               ...
               if (rc_tlp_q.size() > 0) begin
                    ...
                    rc_tlp_burst_q.push_back(rc_tlp_q);
               end
          end
          ...
          foreach (rc_dllp_packet[i])
               `uvm_info(
                   "MAC_TX_DRV", $sformatf(
                   "[%s] RC_DLLP_MAC_TX %h %d DWORDs", tag, rc_dllp_packet[i], rc_dllp_packet.size()
                   ), UVM_HIGH)
     ...
     function void write_port_f(Sequence_item t);
          ...
          `uvm_info("MAC_TX_DRV", $sformatf("RC_ORDERED_SETS %0d DWORDs", rc_received_os.size()),
                    UVM_HIGH)
          if (t.tlp_queue.size() > 0) rc_tlp_fifo.push_back(t.tlp_queue);
          if (t.dlp_queue.size() > 0) rc_dllp_fifo.push_back(t.dlp_queue);
          foreach (rc_dllp_queue[i])
               `uvm_info("MAC_TX_DRV", $sformatf(
                         "[%s] RC_DLLP_Received_EP %0h DWORDs", tag, rc_dllp_queue[i]), UVM_HIGH)
          t.dlp_queue.delete();
     endfunction
     ...
     function void write_port_g(Sequence_item t);
          ...
          // here, per row, then push it as its OWN entry into ep_tlp_burst_q
          // (instead of flattening into the single-dimension ep_received queue)
          // so ep_state_l0() drives each TLP as its own separate PIPE burst
          // rather than concatenating whatever piled up into one TxDataValid
          // assertion.
          foreach (t.mac_rx_data[i]) begin
               ...
               if (ep_tlp_q.size() > 0) begin
                    ...
                    ep_tlp_burst_q.push_back(ep_tlp_q);
               end
          end
          ...
          foreach (ep_dllp_packet[i])
               `uvm_info(
                   "MAC_RX_DRV", $sformatf(
                   "[%s] EP_DLLP_MAC %h %d DWORDs", tag, ep_dllp_packet[i], ep_dllp_packet.size()),
                   UVM_HIGH)
     endfunction
     ...
     function void write_port_h(Sequence_item t);
          ...
          if (t.tlp_queue_t.size() > 0) ep_tlp_fifo.push_back(t.tlp_queue_t);
          if (t.dlp_rx_queue.size() > 0) ep_dllp_fifo.push_back(t.dlp_rx_queue);
          foreach (t.tlp_queue_t[i])
               `uvm_info("MAC_RX_DRV", $sformatf(
                         "[%s] Received from PMA DRIVER %0h %d DWORDs",
                         tag,
                         t.tlp_queue_t[i],
                         t.tlp_queue_t.size()
                         ), UVM_HIGH)
     ...
     function bit parity_calc(input length, input sequence_number);
     ...
     function void os_scramble(inout bit [127:0] os);
          for (int i = 8; i < 113; i = i + 8) os[i+:8] = rc_d_s(os[i+:8]);
     endfunction
     ...
     function void rc_packet_encoding();
          ...
          if (rc_received.size() % 4 == 1) begin
               ...
               rc_received.push_back(32'h0);
               rc_received.push_back(32'h0);
          end else if (rc_received.size() % 4 == 2) begin
               rc_received.push_back(32'h0);
               rc_received.push_back(32'h0);
          end else if (rc_received.size() % 4 == 3) begin
               rc_received.push_back(32'h0);
          end
          foreach (rc_received[i])
               `uvm_info("PIPE_DRV", $sformatf(
                         "PACKET ENCODING: %0h size = %d", rc_received[i], rc_received.size()),
                         UVM_HIGH)
     ...
     function bit [7:0] rc_lane_scramble_byte(input int lane, inout bit [7:0] data,
                                              input bit rst = 1'b0);
     ...
     task rc_byte_striping(input bit [31:0] rc_received[$]);
          ...
          `uvm_info("MAC_TX_DRV", $sformatf(
                                      "[%s] RC byte-striping packet: %0d DWs across %0d lanes",
                                      tag, rc_received.size(), cfg.num_lanes), UVM_LOW)

          payload_bytes.delete();
          foreach (rc_received[w]) begin
               word = rc_received[w];
               for (int b = 3; b >= 0; b--) payload_bytes.push_back(word[b*8+:8]);
          end
          ...
          for (int blk = 0; blk < blocks_per_lane; blk++) begin
               for (int lane = 0; lane < cfg.num_lanes; lane++) begin
                    ...
                    `uvm_info("TX_BYTE_STRIPE", $sformatf(
                              "Block[%0d] Lane[%0d] = %033h", blk, lane, {2'b10, payload}), UVM_LOW)
               end
          end
     endtask
     ...
     task rc_drive_striped_data();
          ...
          `uvm_info("MAC_TX_DRV", $sformatf("[%s] Driving %0d striped block(s) onto PIPE TX lanes",
                                            tag, num_blocks), UVM_LOW)
          ...
          @(posedge rc_pvif.PCLK);
          for (int lane = 0; lane < cfg.num_lanes; lane++) rc_pvif.TxDataValid[lane] <= 0;
...
*/
     ...
     task automatic rc_tx_bcast_data(bit [129:0] data);
          for (int lane = 0; lane < cfg.num_lanes; lane++)
               if (cfg.active_lane_mask[lane]) rc_pvif.TxData[lane] <= data;
     endtask

     task automatic rc_tx_bcast_valid(bit valid);
          for (int lane = 0; lane < cfg.num_lanes; lane++)
               if (cfg.active_lane_mask[lane]) rc_pvif.TxDataValid[lane] <= valid;
     endtask

     task automatic ep_tx_bcast_data(bit [129:0] data);
          for (int lane = 0; lane < cfg.num_lanes; lane++)
               if (cfg.active_lane_mask[lane]) ep_pvif.TxData[lane] <= data;
     endtask

     task automatic ep_tx_bcast_valid(bit valid);
          for (int lane = 0; lane < cfg.num_lanes; lane++)
               if (cfg.active_lane_mask[lane]) ep_pvif.TxDataValid[lane] <= valid;
     endtask
     ...
     task automatic rc_ltssm_timer(input time timeout);
          time start_time;
          start_time = $time;
          while (($time - start_time) < timeout) @(posedge rc_pvif.PCLK);
     endtask

     task automatic ep_ltssm_timer(input time timeout);
          time start_time;
          start_time = $time;
          while (($time - start_time) < timeout) @(posedge ep_pvif.PCLK);
     endtask
     function automatic bit rc_os_all_lanes_ready(int unsigned min_count = 1);
          rc_os_all_lanes_ready = 1'b1;
          for (int lane = 0; lane < cfg.num_lanes; lane++)
          if (rc_received_os_lane[lane].size() < min_count) rc_os_all_lanes_ready = 1'b0;
     endfunction

     function automatic bit ep_os_all_lanes_ready(int unsigned min_count = 1);
          ep_os_all_lanes_ready = 1'b1;
          for (int lane = 0; lane < cfg.num_lanes; lane++)
          if (ep_os_lane[lane].size() < min_count) ep_os_all_lanes_ready = 1'b0;
     endfunction

     function automatic void rc_os_lane_delete();
          for (int lane = 0; lane < cfg.num_lanes; lane++) rc_received_os_lane[lane].delete();
     endfunction

     function automatic void ep_os_lane_delete();
          for (int lane = 0; lane < cfg.num_lanes; lane++) ep_os_lane[lane].delete();
     endfunction
     ...
     task automatic rc_perform_receiver_detection(output bit [`PCIE_NUM_LANES-1:0] detect_mask);
                    ...
                    if (cfg.active_lane_mask[l])
                         fork
                              begin
                                   ...
                                   if (rc_pvif.RxStatus[l] == RX_DETECTED) detect_mask[l] = 1'b1;
          ...
          for (int lane = 0; lane < cfg.num_lanes; lane++)
               if (cfg.active_lane_mask[lane]) wait (!rc_pvif.PhyStatus[lane]);
     ...
     task automatic ep_perform_receiver_detection(output bit [`PCIE_NUM_LANES-1:0] detect_mask);
                    ...
                    if (cfg.active_lane_mask[l])
                         fork
                              begin
                                   ...
                                   if (ep_pvif.RxStatus[l] == RX_DETECTED) detect_mask[l] = 1'b1;
          ...
          for (int lane = 0; lane < cfg.num_lanes; lane++)
               if (cfg.active_lane_mask[lane]) wait (!ep_pvif.PhyStatus[lane]);
     ...
     function void ep_packet_encoding();
          ...
          if (ep_received.size() % 4 == 1) begin
               ...
               ep_received.push_back(32'h0);
               ep_received.push_back(32'h0);
          end else if (ep_received.size() % 4 == 2) begin
               ep_received.push_back(32'h0);
               ep_received.push_back(32'h0);
          end else if (ep_received.size() % 4 == 3) begin
               ep_received.push_back(32'h0);
          end
          `uvm_info("PIPE_DRV", $sformatf(
                    "PACKET ENCODING: %0p size = %d", ep_received, ep_received.size()), UVM_HIGH)
     ...
     // scrambling a byte.
     function bit [7:0] ep_lane_scramble_byte(input int lane, inout bit [7:0] data,
                                              input bit rst = 1'b0);
          static bit [22:0] lfsr          [`PCIE_NUM_LANES];
          static bit        seeded = 1'b0;
          if (!seeded) begin
     ...
     task ep_byte_striping(input bit [31:0] ep_received[$]);
          ...
          `uvm_info("MAC_TX_DRV", $sformatf(
                                      "[%s] EP byte-striping packet: %0d DWs across %0d lanes",
          ...
          for (int blk = 0; blk < blocks_per_lane; blk++) begin
               for (int lane = 0; lane < cfg.num_lanes; lane++) begin
                    ...
                    `uvm_info("EP_BYTE_STRIPE", $sformatf(
                              "Block[%0d] Lane[%0d] = %033h", blk, lane, {2'b10, payload}),
                              UVM_HIGH)
               end
          end
     endtask
     ...
     task ep_drive_striped_data();
          ...
          @(posedge ep_pvif.PCLK);
          for (int lane = 0; lane < cfg.num_lanes; lane++) ep_pvif.TxDataValid[lane] <= 0;
...
*/
     ...
     task rc_state_detect_active();
          ...
          `uvm_info("TX_LTSSM", "Entered DETECT_ACTIVE", UVM_HIGH)
          ...
          if (timeout_occured && !detection_done) begin
          ...
          if (rc_detect_mask_first == cfg.active_lane_mask) begin

               `uvm_info("TX_LTSSM", "Receiver detected on all active lanes", UVM_HIGH)

               for (int lane = 0; lane < cfg.num_lanes; lane++)
               if (cfg.active_lane_mask[lane]) rc_pvif.TxElecIdle[lane] <= 1'b0;
          ...
          if (rc_detect_mask_first == '0) begin
          ...
          `uvm_info("TX_LTSSM", "Partial lane detection. Retrying...", UVM_HIGH)

          rc_ltssm_timer(cfg.detect_retry_timeout);
          rc_perform_receiver_detection(rc_detect_mask_second);

          if (rc_detect_mask_first == rc_detect_mask_second) begin

               `uvm_info("TX_LTSSM", "Stable lane detection", UVM_HIGH)

               for (int lane = 0; lane < cfg.num_lanes; lane++) begin
                    if (!cfg.active_lane_mask[lane]) continue;
                    if (rc_detect_mask_second[lane]) rc_pvif.TxElecIdle[lane] <= 1'b0;
                    else rc_pvif.TxElecIdle[lane] <= 1'b1;  // no Receiver - hold Elec Idle
               end

               rc_state = POLLING_ACTIVE;

          end else begin
               `uvm_info("TX_LTSSM", "Lane detection changed. Returning to Detect.Quiet", UVM_HIGH)
               rc_state = DETECT_QUIET;
          end
     ...
     task ep_state_detect_active();
          ...
          `uvm_info("RX_LTSSM", "Entered DETECT_ACTIVE", UVM_HIGH)
          ...
          if (ep_detect_mask_first == cfg.active_lane_mask) begin

               `uvm_info("RX_LTSSM", "Receiver detected on all active lanes", UVM_HIGH)

               for (int lane = 0; lane < cfg.num_lanes; lane++)
               if (cfg.active_lane_mask[lane]) ep_pvif.TxElecIdle[lane] <= 1'b0;
          ...
          if (ep_detect_mask_first == '0) begin
          ...
          if (ep_detect_mask_first == ep_detect_mask_second) begin

               `uvm_info("RX_LTSSM", "Stable lane detection", UVM_HIGH)

               for (int lane = 0; lane < cfg.num_lanes; lane++) begin
                    if (!cfg.active_lane_mask[lane]) continue;
                    if (ep_detect_mask_second[lane]) ep_pvif.TxElecIdle[lane] <= 1'b0;
                    else ep_pvif.TxElecIdle[lane] <= 1'b1;
               end

               ep_state = EP_POLLING_ACTIVE;

          end else begin
               `uvm_info("RX_LTSSM", "Lane detection changed. Returning to Detect.Quiet", UVM_HIGH)
               ep_state = EP_DETECT_QUIET;
          end
     ...
     task rc_state_polling_active();
          ...
          `uvm_info("TX_LTSSM", $sformatf("Entered POLLING_ACTIVE (advertising Gen%0d)", cfg.gen),
                    UVM_LOW)

          // Immediate Polling.Compliance if Enter Compliance bit was
          // already set prior to entry - no TS1 sent.

          if (cfg.link_ctrl2_enter_compliance) begin
               `uvm_info("TX_LTSSM",
                         "Enter Compliance bit set prior to entry -> immediate POLLING_COMPLIANCE",
                         UVM_HIGH)
          ...
          fork
               ...
               begin
                    ...
                    `uvm_info("MAC_TX_DRV", $sformatf(
                              " wait_completetd RC_ORDERED_SETS %0d DWORDs", rc_received_os.size()),
                              UVM_LOW)
          ...
          if (enter_compliance) begin
               `uvm_info("TX_LTSSM",
                         "Enter Compliance bit set during POLLING_ACTIVE -> POLLING_COMPLIANCE",
                         UVM_HIGH)
               rc_os_lane_delete();
               rc_state = POLLING_COMPLIANCE;
               return;
          ...
          if (barrier_met) begin
               ...
               rc_negotiated_gen = (cfg.gen < decode_max_gen(rc_partner_rate_id)) ? cfg.gen :
                   decode_max_gen(rc_partner_rate_id);

               `uvm_info(
                   "TX_LTSSM",
                   $sformatf(
                       "Rate negotiation: own Gen%0d, partner rate_id=%08b (max Gen%0d) -> negotiated Gen%0d",
                       cfg.gen, rc_partner_rate_id, decode_max_gen(rc_partner_rate_id),
                       rc_negotiated_gen), UVM_LOW)

               `uvm_info(
                   "TX_LTSSM",
                   ">=1024 TS1 sent, 8 consecutive matching TS received -> POLLING_CONFIGURATION",
                   UVM_HIGH)
               rc_os_lane_delete();
          ...
          `uvm_info("TX_LTSSM",
                    "24ms timeout without meeting fast-path barrier -> POLLING_COMPLIANCE",
                    UVM_HIGH)
          rc_os_lane_delete();
          rc_state = POLLING_COMPLIANCE;
     ...
     task ep_state_polling_active();
          ...
          `uvm_info("RX_LTSSM", $sformatf("Entered POLLING_ACTIVE (advertising Gen%0d)", cfg.gen),
                    UVM_LOW)
          ...
          fork
               ...
               begin
                    ...
                    `uvm_info("MAC_TX_DRV", $sformatf(
                              " wait_completed ORDERED_SETS %0d DWORDs ts1_sent_count = %d",
                              ep_os.size(),
                              ts1_sent_count
                              ), UVM_LOW)
          ...
          if (barrier_met) begin
               ...
               ep_negotiated_gen = (cfg.gen < decode_max_gen(ep_partner_rate_id)) ? cfg.gen :
                   decode_max_gen(ep_partner_rate_id);

               `uvm_info(
                   "RX_LTSSM",
                   $sformatf(
                       "Rate negotiation: own Gen%0d, partner rate_id=%08b (max Gen%0d) -> negotiated Gen%0d",
                       cfg.gen, ep_partner_rate_id, decode_max_gen(ep_partner_rate_id),
                       ep_negotiated_gen), UVM_LOW)
     ...
     task rc_state_polling_compliance();
          `uvm_info("TX_LTSSM", "POLLING_COMPLIANCE", UVM_HIGH)
          repeat (4) begin
               @(posedge rc_pvif.PCLK);
          ...
          `uvm_info("TX_LTSSM", $sformatf("Compliance %p", rc_received), UVM_HIGH)
          polling_compilance.wait_for();
          `uvm_info("TX_LTSSM", "Compliance exit → POLLING_CONFIGURATION", UVM_HIGH)
          rc_os_lane_delete();
          rc_state = POLLING_CONFIGURATION;
     endtask
     ...
     task ep_state_polling_compliance();
          `uvm_info("RX_LTSSM", "POLLING_COMPLIANCE – waiting compliance_exit", UVM_HIGH)
          wait (ep_os_all_lanes_ready(1));
          polling_compilance.wait_for();
          `uvm_info("RX_LTSSM", $sformatf("Compliance ep_os %p ep_size = %d", ep_os, ep_os.size()),
                    UVM_LOW)
          ep_os_lane_delete();
          `uvm_info("RX_LTSSM", "Compliance exit ep_received → POLLING_CONFIGURATION", UVM_HIGH)
          ep_state = EP_POLLING_CONFIGURATION;
     endtask
     ...
     task rc_state_polling_configuration();
          ...
          `uvm_info("TX_LTSSM", "Entered POLLING_CONFIGURATION", UVM_HIGH)
          ...
          fork
               ...
               begin
                    wait (rc_os_all_lanes_ready(7));
                    `uvm_info("MAC_TX_DRV", $sformatf(
                              "wait_completed RC_ORDERED_SETS %0d DWORDs", rc_received_os.size()),
                              UVM_LOW)
               ...
               begin
                    rc_ltssm_timer(
                        cfg.polling_configuration_timeout);  // TODO: add 48ms timeout param to cfg
                    timeout_occured = 1;
                    disable fork;
               end
          ...
          `uvm_info("TX_LTSSM", "48ms timeout without meeting fast-path barrier -> DETECT",
                    UVM_HIGH)
          rc_os_lane_delete();
          rc_state = DETECT_QUIET;
     ...
     task ep_state_polling_configuration();
          ...
          `uvm_info("TX_LTSSM", "EP Entered POLLING_CONFIGURATION", UVM_HIGH)
          ...
          fork
               ...
               begin
                    ...
                    `uvm_info("MAC_TX_DRV", $sformatf(
                              "wait_completed EP_ORDERED_SETS %0d DWORDs", ep_os.size()), UVM_LOW)
          ...
          // 48ms timeout without meeting the fast-path barrier -> Detect
          `uvm_info("TX_LTSSM", "EP: 48ms timeout without meeting fast-path barrier -> DETECT",
                    UVM_HIGH)
          ep_os_lane_delete();
          ep_state = EP_DETECT_QUIET;
     ...
     task rc_state_config_linknum_start();
          ...
          if (cfg.link_ctrl_disable_link) begin
               `uvm_info("TX_LTSSM", "Link Disable bit set prior to entry -> immediate DISABLED",
                         UVM_HIGH)
               rc_state = DISABLED;
               return;
          end
          ...
          if (cfg.link_ctrl_enter_loopback) begin
               `uvm_info("TX_LTSSM",
                         "Enter Loopback set prior to entry -> immediate LOOPBACK_ENTRY", UVM_HIGH)
          ...
          `uvm_info("TX_LTSSM", $sformatf("CONFIG_LINKNUM_START - broadcasting Link=%0h, Lane=PAD",
                                          rc_negotiated_link_num), UVM_LOW)
          ...
          if (cfg.link_ctrl_disable_link) begin
               `uvm_info("TX_LTSSM", "Link Disable bit set -> DISABLED", UVM_HIGH)
               rc_os_lane_delete();
               rc_state = DISABLED;
               return;
          end
          ...
          if (cfg.link_ctrl_enter_loopback) begin
               `uvm_info("TX_LTSSM", "Enter Loopback set -> LOOPBACK_ENTRY", UVM_HIGH)
               rc_os_lane_delete();
               rc_state = LOOPBACK_ENTRY;
               return;
          end
          ...
          if (barrier_met) begin
               ...
               `uvm_info("TX_LTSSM", $sformatf(
                                         "CONFIG_LINKNUM_START -> negotiating | rc_link_num=%0h",
                                         rc_link_num), UVM_LOW)
               `uvm_info("TX_LTSSM", "Link num broadcast done -> CONFIG_LINKNUM_ACCEPT", UVM_HIGH)
               rc_os_lane_delete();
               rc_state = CONFIG_LINKNUM_ACCEPT;
               return;
          end

          `uvm_info("TX_LTSSM", "24ms timeout in CONFIG_LINKNUM_START -> DETECT_QUIET", UVM_HIGH)
          rc_os_lane_delete();
          rc_state = DETECT_QUIET;
     endtask
     ...
     task ep_state_config_linknum_start();
          ...
          bit timeout_occured;
          if (cfg.link_ctrl_disable_link) begin
               `uvm_info("RX_LTSSM", "Link Disable bit set prior to entry -> immediate DISABLED",
                         UVM_HIGH)
          ...
          `uvm_info("RX_LTSSM", "CONFIG_LINKNUM_START - waiting for Downstream Link num", UVM_HIGH)
          ...
          if (cfg.link_ctrl_disable_link) begin
               `uvm_info("RX_LTSSM", "Link Disable bit set -> DISABLED", UVM_HIGH)
               ep_os_lane_delete();
               ep_state = EP_DISABLED;
               return;
          end
          ...
          if (cfg.link_ctrl_enter_loopback) begin
               `uvm_info("RX_LTSSM", "Enter Loopback set -> LOOPBACK_ENTRY", UVM_HIGH)
               ep_os_lane_delete();
               ep_state = EP_LOOPBACK_ENTRY;
               return;
          end
          ...
          if (barrier_met) begin
               ...
               `uvm_info("RX_LTSSM", $sformatf(
                                         "CONFIG_LINKNUM_START -> negotiating | ep_link_num=%0h",
                                         ep_link_num), UVM_LOW)
               `uvm_info("RX_LTSSM", "Link num broadcast done -> CONFIG_LINKNUM_ACCEPT", UVM_HIGH)
               ep_os_lane_delete();
               ep_state = EP_CONFIG_LINKNUM_ACCEPT;
               return;
          end

          `uvm_info("RX_LTSSM", "24ms timeout in CONFIG_LINKNUM_START -> DETECT_QUIET", UVM_HIGH)
          ep_os_lane_delete();
          ep_state = EP_DETECT_QUIET;
     endtask
     ...
     task rc_state_config_linknum_accept();
          ...
          `uvm_info("TX_LTSSM", $sformatf("CONFIG_LINKNUM_ACCEPT - assigning Link=%0h Lane=%0h",
                                          rc_negotiated_link_num, rc_negotiated_lane_num), UVM_LOW)
          ...
          fork
               begin
                    repeat (2) begin
                         ...
                         rc_drive_os(tag_ts1(
                                     rc_TS1,
                                     .link_num(rc_negotiated_link_num),
                                     .lane_num(rc_negotiated_lane_num)
                                     ));
                    end
                    @(posedge rc_pvif.PCLK);
                    rc_tx_bcast_valid(0);
          ...
          if (barrier_met) begin
               ...
               `uvm_info(
                   "TX_LTSSM",
                   $sformatf(
                       "CONFIG_LINKNUM_ACCEPT -> confirmed | rc_link_num=%0h, rc_lane_num=%0h",
                       rc_link_num, rc_lane_num), UVM_LOW)
               rc_os_lane_delete();
               `uvm_info("TX_LTSSM", "Link-num & Lane_num accepted -> CONFIG_LANENUM_WAIT",
                         UVM_HIGH)
               rc_state = CONFIG_LANNUM_WAIT;
               return;
          end

          `uvm_info("TX_LTSSM", "2ms timeout in CONFIG_LINKNUM_ACCEPT -> DETECT_QUIET", UVM_HIGH)
          rc_os_lane_delete();
          rc_state = DETECT_QUIET;
     endtask
     ...
     task ep_state_config_linknum_accept();
          ...
          `uvm_info("RX_LTSSM", $sformatf("CONFIG_LINKNUM_ACCEPT - assigning Link=%0h Lane=%0h",
                                          ep_negotiated_link_num, ep_negotiated_lane_num), UVM_LOW)
          ...
          fork
               begin
                    repeat (2) begin
                         ...
                         ep_drive_os(tag_ts1(
                                     ep_TS1,
                                     .link_num(ep_negotiated_link_num),
                                     .lane_num(ep_negotiated_lane_num)
                                     ));
                    end
                    @(posedge ep_pvif.PCLK);
                    ep_tx_bcast_valid(0);
          ...
          if (barrier_met) begin
               ...
               `uvm_info(
                   "RX_LTSSM",
                   $sformatf(
                       "CONFIG_LINKNUM_ACCEPT -> confirmed | ep_link_num=%0h, ep_lane_num=%0h",
                       ep_link_num, ep_lane_num), UVM_LOW)
               ep_os_lane_delete();
               `uvm_info("RX_LTSSM", "Link-num accepted -> CONFIG_LANENUM_WAIT", UVM_HIGH)
               ep_state = EP_CONFIG_LANNUM_WAIT;
               return;
          end

          `uvm_info("RX_LTSSM", "2ms timeout in CONFIG_LINKNUM_ACCEPT -> DETECT_QUIET", UVM_HIGH)
          ep_os_lane_delete();
          ep_state = EP_DETECT_QUIET;
     endtask
     ...
     task rc_state_config_lanenum_wait();
          ...
          `uvm_info("TX_LTSSM", $sformatf("CONFIG_LANENUM_WAIT - Lane on entry=%0h",
                                          rc_lane_num_on_entry), UVM_LOW)
          ...
          fork
               ...
               begin
                    rc_ltssm_timer(
                        cfg.config_lanenum_wait_timeout);  // TODO: add 2ms timeout param to cfg
                    timeout_occured = 1;
                    disable fork;
               end
          join
          ...
          if (barrier_met) begin
               `uvm_info(
                   "TX_LTSSM",
                   $sformatf(
                       "CONFIG_LANENUM_WAIT -> negotiating | rc_link_num=%0h, rc_lane_num=%0h",
                       rc_link_num, rc_lane_num), UVM_LOW)
               rc_os_lane_delete();
               `uvm_info("TX_LTSSM", "Lane numbers rc_received -> CONFIG_LANENUM_ACCEPT", UVM_HIGH)
               rc_state = CONFIG_LANENUM_ACCEPT;
               return;
          end

          // 2ms timeout, or all-Lanes-PAD exit -> Detect
          `uvm_info("TX_LTSSM", "2ms timeout / all-PAD in CONFIG_LANENUM_WAIT -> DETECT_QUIET",
                    UVM_HIGH)
          rc_os_lane_delete();
          rc_state = DETECT_QUIET;
     endtask
     ...
     task ep_state_config_lanenum_wait();
          ...
          `uvm_info("RX_LTSSM", $sformatf("CONFIG_LANENUM_WAIT - Lane on entry=%0h",
                                          ep_lane_num_on_entry), UVM_LOW)
          ...
          if (barrier_met) begin
               `uvm_info(
                   "RX_LTSSM",
                   $sformatf(
                       "CONFIG_LANENUM_WAIT -> negotiating | ep_link_num=%0h, ep_lane_num=%0h",
                       ep_link_num, ep_lane_num), UVM_LOW)
               ep_os_lane_delete();
               `uvm_info("RX_LTSSM", "Lane broadcast done -> CONFIG_LANENUM_ACCEPT", UVM_HIGH)
               ep_state = EP_CONFIG_LANENUM_ACCEPT;
               return;
          end

          `uvm_info("RX_LTSSM", "2ms timeout / all-PAD in CONFIG_LANENUM_WAIT -> DETECT_QUIET",
                    UVM_HIGH)
          ep_os_lane_delete();
          ep_state = EP_DETECT_QUIET;
     endtask
     ...
     task rc_state_config_lanenum_accept();
          ...
          `uvm_info("TX_LTSSM", $sformatf("CONFIG_LANENUM_ACCEPT - confirming Link=%0h Lane=%0h",
                                          rc_negotiated_link_num, rc_negotiated_lane_num), UVM_LOW)
          ...
          fork
               ...
               begin
                    rc_ltssm_timer(
                        cfg.config_lanenum_accept_timeout);  // TODO: add 2ms timeout param to cfg
                    timeout_occured = 1;
                    disable fork;
               end
          join
          ...
          if (full_match) begin
               `uvm_info(
                   "TX_LTSSM",
                   $sformatf(
                       "CONFIG_LANENUM_ACCEPT -> confirmed | rc_link_num=%0h, rc_lane_num=%0h",
                       rc_link_num, rc_lane_num), UVM_LOW)
               rc_os_lane_delete();
               `uvm_info("TX_LTSSM", "Lane-num accepted -> CONFIG_COMPLETE", UVM_HIGH)
               rc_state = CONFIG_COMPLETE;
               return;
          end

          `uvm_info("TX_LTSSM",
                    "No Link configurable / all-PAD in CONFIG_LANENUM_ACCEPT -> DETECT_QUIET",
                    UVM_HIGH)
          rc_os_lane_delete();
          rc_state = DETECT_QUIET;
     endtask
     ...
     task ep_state_config_lanenum_accept();
          ...
          `uvm_info("RX_LTSSM", $sformatf("CONFIG_LANENUM_ACCEPT - confirming Link=%0h Lane=%0h",
                                          ep_negotiated_link_num, ep_negotiated_lane_num), UVM_LOW)
          ...
          if (full_match) begin
               `uvm_info(
                   "RX_LTSSM",
                   $sformatf(
                       "CONFIG_LANENUM_ACCEPT -> confirmed | ep_link_num=%0h, ep_lane_num=%0h",
                       ep_link_num, ep_lane_num), UVM_LOW)
               ep_os_lane_delete();
               `uvm_info("RX_LTSSM", "Lane-num accepted -> CONFIG_COMPLETE", UVM_HIGH)
     ...
     task rc_state_config_complete();
          ...
          `uvm_info("TX_LTSSM",
                    $sformatf("Entered CONFIG_COMPLETE (advertising Gen%0d, Link=%0h Lane=%0h)",
                              cfg.gen, rc_negotiated_link_num, rc_negotiated_lane_num), UVM_LOW)
          ...
          fork
               ...
               begin
                    repeat (16) begin
                         ...
                         if (rc_first_ts2_rx_seen) rc_ts2_sent_since_first_rx++;
                    end
                    @(posedge rc_pvif.PCLK);
                    rc_tx_bcast_valid(0);
               end
               ...
               begin
                    ...
                    `uvm_info(
                        "MAC_TX_DRV", $sformatf(
                        "wait_completed RC_ORDERED_SETS %0d DWORDs", rc_received_os_lane[0].size()),
                        UVM_LOW)
               ...
               begin
                    rc_ltssm_timer(
                        cfg.config_complete_timeout);  // TODO: add 2ms timeout param to cfg
                    timeout_occured = 1;
                    disable fork;
               end
          join
          ...
          if (idle_ready) begin
               `uvm_info("TX_LTSSM",
                         "8x consecutive TS2 match + 16 sent-after-first -> CONFIG_IDLE", UVM_HIGH)
          ...
          if (cfg.gen == 1 || cfg.gen == 2) begin
               `uvm_info("TX_LTSSM", "2ms timeout at Gen1/Gen2 -> DETECT_QUIET", UVM_HIGH)
               rc_state = DETECT_QUIET;
          end else if (cfg.gen == 3 && rc_idle_to_rlock_transitioned < 8'hFF) begin
               `uvm_info("TX_LTSSM",
                         "2ms timeout, Gen3, idle_to_rlock_transitioned<FFh -> CONFIG_IDLE",
                         UVM_HIGH)
               rc_state = CONFIG_IDLE;
          end else begin
               `uvm_info("TX_LTSSM", "2ms timeout, fallback -> DETECT_QUIET", UVM_HIGH)
               rc_state = DETECT_QUIET;
          end
     endtask
     ...
     task ep_state_config_complete();
          ...
          `uvm_info("RX_LTSSM",
                    $sformatf("Entered CONFIG_COMPLETE (advertising Gen%0d, Link=%0h Lane=%0h)",
                              cfg.gen, ep_negotiated_link_num, ep_negotiated_lane_num), UVM_LOW)
          ...
          fork
               ...
               begin
                    repeat (8) begin
                         ...
                         if (ep_first_ts2_rx_seen) ep_ts2_sent_since_first_rx++;
                    end
                    @(posedge ep_pvif.PCLK);
                    ep_tx_bcast_valid(0);
               end
          ...
          if (cfg.gen == 1 || cfg.gen == 2) begin
               `uvm_info("RX_LTSSM", "2ms timeout at Gen1/Gen2 -> DETECT_QUIET", UVM_HIGH)
               ep_state = EP_DETECT_QUIET;
          end else if (cfg.gen == 3 && ep_idle_to_rlock_transitioned < 8'hFF) begin
               `uvm_info("RX_LTSSM",
                         "2ms timeout, Gen3, idle_to_rlock_transitioned<FFh -> CONFIG_IDLE",
                         UVM_HIGH)
               ep_state = EP_CONFIG_IDLE;
          end else begin
               `uvm_info("RX_LTSSM", "2ms timeout, fallback -> DETECT_QUIET", UVM_HIGH)
               ep_state = EP_DETECT_QUIET;
          end
     endtask
     ...
     task rc_state_config_idle();
          ...
          `uvm_info("TX_LTSSM", "Entered CONFIG_IDLE - sending Idle data Symbols", UVM_HIGH)
          ...
          fork
               ...
               begin
                    ...
                    `uvm_info("TX_LTSSM", $sformatf(
                              "Received rc_idle activity on all %0d active lanes in CONFIG_IDLE",
                              cfg.num_lanes
                              ), UVM_LOW)
                    l0_ready = 1;
                    disable fork;
               ...
               begin
                    rc_ltssm_timer(
                        cfg.config_idle_timeout);  // TODO: add 2ms min timeout param to cfg
                    timeout_occured = 1;
                    disable fork;
               end
          join
          ...
          if (l0_ready) begin
               `uvm_info("TX_LTSSM", "8x consecutive Idle + 16 sent-after-first -> L0", UVM_HIGH)
          ...
          if (rc_idle_to_rlock_transitioned < 8'hFF) begin
               `uvm_info("TX_LTSSM",
                         $sformatf(
                             "2ms timeout, idle_to_rlock_transitioned=%0h<FFh -> RECOVERY_RCVRLOCK",
                             rc_idle_to_rlock_transitioned), UVM_LOW)
               if (cfg.gen == 3)
                    rc_idle_to_rlock_transitioned = rc_idle_to_rlock_transitioned + 1'b1;
               else rc_idle_to_rlock_transitioned = 8'hFF;
               rc_state = RECOVERY_RCVRLOCK;
          end else begin
               `uvm_info("TX_LTSSM", "2ms timeout, idle_to_rlock_transitioned=FFh -> DETECT_QUIET",
                         UVM_HIGH)
               rc_state = DETECT_QUIET;
          end
     endtask
     ...
     task ep_state_config_idle();
          ...
          `uvm_info("RX_LTSSM", "Entered CONFIG_IDLE - sending Idle data Symbols", UVM_HIGH)
          ...
          fork
               ...
               begin
                    ...
                    config_idle.wait_for();
                    `uvm_info("RX_LTSSM", $sformatf(
                              "Received ep_idle activity on all %0d active lanes in CONFIG_IDLE",
                              cfg.num_lanes
                              ), UVM_LOW)
                    l0_ready = 1;
                    disable fork;
          ...
          if (l0_ready) begin
               `uvm_info("RX_LTSSM", "8x consecutive Idle + 16 sent-after-first -> L0", UVM_HIGH)
          ...
          if (ep_idle_to_rlock_transitioned < 8'hFF) begin
               `uvm_info("RX_LTSSM",
                         $sformatf(
                             "2ms timeout, idle_to_rlock_transitioned=%0h<FFh -> RECOVERY_RCVRLOCK",
                             ep_idle_to_rlock_transitioned), UVM_LOW)
               if (cfg.gen == 3)
                    ep_idle_to_rlock_transitioned = ep_idle_to_rlock_transitioned + 1'b1;
               else ep_idle_to_rlock_transitioned = 8'hFF;
               ep_state = EP_RECOVERY_RCVRLOCK;
          end else begin
               `uvm_info("RX_LTSSM", "2ms timeout, idle_to_rlock_transitioned=FFh -> DETECT_QUIET",
                         UVM_HIGH)
               ep_state = EP_DETECT_QUIET;
          end
     endtask
     ...
     // ---- RC_MODE ----
     task rc_state_l0();
          `uvm_info("TX_LTSSM", "L0 – link is up", UVM_HIGH)
          ...
          if (rc_negotiated_gen > rc_active_gen) begin
               ...
               fork
                    forever begin
                         begin
                              wait (rc_tlp_burst_q.size() > 0 || rc_dllp_packet.size() > 0);
                              `uvm_info("PIPE_DRV", $sformatf(
                                        "dllp_size=%0d tlp_burst_q_size=%0d",
                                        rc_dllp_packet.size(),
                                        rc_tlp_burst_q.size()
                                        ), UVM_HIGH)
                              ...
                              if (rc_dllp_packet.size() > 0) begin
                                   foreach (rc_dllp_packet[i])
                                        `uvm_info("PIPE_DRV", $sformatf(
                                                  "DLLP_INITIAL: %0h", rc_dllp_packet[i]), UVM_HIGH)

                                   rc_sdp_packet = {
                                        SDP[15:0], rc_dllp_packet[0][31:0], rc_dllp_packet[1][31:16]
                                   };
                                   `uvm_info("PIPE_DRV", $sformatf("SDP: %0h", rc_sdp_packet),
                                             UVM_HIGH)
                                   ...
                                   //        end
                                   //         @(posedge rc_pvif.PCLK);
                              end else if (rc_tlp_burst_q.size() > 0) begin
                                   // Pop exactly ONE already-STP-framed TLP off the front of
                                   // rc_tlp_burst_q and drive ONLY that TLP through byte-striping +
                                   // rc_drive_striped_data(). rc_drive_striped_data() asserts
                                   // TxDataValid for this TLP's blocks and then deasserts it before
                                   // returning, so each TLP now appears on the wire as its own
                                   // separate PIPE burst instead of being concatenated with whatever
                                   // else had piled up in the (now-removed) flat rc_received queue.
                                   bit [31:0] rc_one_tlp_q[$];
                                   rc_one_tlp_q = rc_tlp_burst_q.pop_front();

                                   `uvm_info("PIPE_DRV", $sformatf("DATA BEFORE_Srcambled: %0d",
                                                                   rc_one_tlp_q.size()), UVM_HIGH)
                                   foreach (rc_one_tlp_q[i])
                                        `uvm_info("PIPE_DRV", $sformatf(
                                                  "RC_SCRAMBLED_BEFORE %0h", rc_one_tlp_q[i]),
                                                  UVM_HIGH)
                                   // NOTE: this TLP already carries its own STP - it was added per-TLP
                                   // in write_port_e() before being pushed into rc_tlp_burst_q. Do NOT
                                   // call rc_add_stp_to_all_packets() here again.
                                   rc_one_tlp_q.push_back(EDS);
                                   foreach (rc_one_tlp_q[i])
                                        `uvm_info("PIPE_DRV", $sformatf(
                                                  "RC_SCRAMBLED_AFTER %0h", rc_one_tlp_q[i]),
                                                  UVM_LOW)

                                   rc_byte_striping(rc_one_tlp_q);
                         ...
                         begin
                              ...
                              if (rc_tlp_fifo.size() > 0) begin
                                   ...
                                   foreach (rc_received_tlp[i])
                                        `uvm_info("MAC_TX_DRV", $sformatf(
                                                  "[%s] Compeletion_Received %0h %d  DWORDs",
                                                  tag,
                                                  rc_received_tlp[i],
                                                  rc_size
                                                  ), UVM_HIGH)
                                   ...
                                   for (int i = 0; i < rc_size_rx; i++) begin
                                        ...
                                        `uvm_info("MAC_TX_DRV", $sformatf(
                                                  "[%s] Compeletion_Received_i %d  value %0h DWORDs",
                                                  tag,
                                                  i,
                                                  rc_vif.dl_rx_data
                                                  ), UVM_HIGH)
                                   ...
                                   foreach (rc_dllp_fifo[i])
                                        `uvm_info("MAC_TX_DRV", $sformatf(
                                                  "RC_DLLP_Received_1 %0p DWORDs ", rc_dllp_fifo[i]
                                                  ), UVM_HIGH)
                                   rc_dllp_queue = rc_dllp_fifo.pop_front();
                                   foreach (rc_dllp_queue[i])
                                        `uvm_info("MAC_TX_DRV", $sformatf(
                                                  "RC_DLLP_Received_1 %0p DWORDs ", rc_dllp_queue[i]
                                                  ), UVM_HIGH)
     ...
     // ---- EP_MODE ----
     task ep_state_l0();
          `uvm_info("RX_LTSSM", "L0 - link is up", UVM_HIGH)
          ...
          if (ep_negotiated_gen > ep_active_gen) begin
               ...
               `uvm_info("RX_LTSSM", $sformatf(
                         "Speed match: running Gen%0d, negotiated Gen%0d -> RECOVERY",
                         ep_active_gen,
                         ep_negotiated_gen
                         ), UVM_LOW)
               fork
                    forever begin
                         ...
                         if (ep_dllp_fifo.size() > 0) begin
                              ...
                              foreach (ep_dllp_queue[i])
                                   `uvm_info("MAC_RX_DRV", $sformatf(
                                             "EP_DLLP_Received_1 %0h DWORDs ", ep_dllp_queue[i]),
                                             UVM_HIGH)
                              foreach (ep_dllp_queue[i])
                                   `uvm_info("MAC_RX_DRV", $sformatf(
                                             "EP_DLLP_Received_1 %0h DWORDs %d ",
                                             ep_dllp_queue[i],
                                             ep_size
                                             ), UVM_HIGH)
                         ...
                         end else if (ep_tlp_fifo.size() > 0) begin
                              ep_received_tlp = ep_tlp_fifo.pop_front();
                              `uvm_info("MAC_RX_DRV", $sformatf("TLP_Received_1 %p  %0d DWORDs ",
                                                                ep_received_tlp,
                                                                ep_received_tlp.size()), UVM_HIGH)
                              ep_size_tlp = ep_received_tlp.size();
                              foreach (ep_received_tlp[i])
                                   `uvm_info("MAC_RX_DRV", $sformatf(
                                             "VALID_TO_ONE ep_received_tlp = %h", ep_received_tlp[i]
                                             ), UVM_HIGH)
                              for (int i = 0; i < ep_size_tlp; i++) begin
                                   ...
                                   `uvm_info("MAC_RX_DRV", $sformatf(
                                             "VALID_TO_ONE i=%d ep_size = %d ep_received_tlp = %h",
                                             i,
                                             ep_size_tlp,
                                             ep_tlp_dword
                                             ), UVM_HIGH)
                              ...
                              @(posedge ep_vif.CLK);
                              `uvm_info("MAC_RX_DRV", $sformatf(
                                                          "VALID_TO_ZERO ep_received_tlp = %d",
                                                          ep_received_tlp.size()), UVM_HIGH)
                              ep_vif.dl_rx_valid   <= 0;
                    ...
                    forever begin
                         begin
                              wait (ep_tlp_burst_q.size() > 0 || ep_dllp_packet.size() > 0);
                              if (ep_dllp_packet.size() > 0) begin
                                   foreach (ep_dllp_packet[i])
                                        `uvm_info("PIPE_DRV", $sformatf(
                                                  "EP_DLLP_MAC_INITIAL: %0h", ep_dllp_packet[i]),
                                                  UVM_HIGH)

                                   ep_sdp_packet = {
                                        SDP[15:0], ep_dllp_packet[0][31:0], ep_dllp_packet[1][31:16]
                                   };
                                   `uvm_info("PIPE_DRV", $sformatf("SDP: %0h", ep_sdp_packet),
                                             UVM_HIGH)
                                   ...
                                   //         end
                                   //          @(posedge ep_pvif.PCLK);
                              end else if (ep_tlp_burst_q.size() > 0) begin
                                   // Pop exactly ONE already-STP-framed TLP and drive only that one,
                                   // asserts/deasserts TxDataValid around a single TLP's blocks, so
                                   // this TLP is a separate PIPE burst instead of being concatenated
                                   // with whatever else had piled up in the (now-removed) flat
                                   // ep_received queue.
                                   bit [31:0] ep_one_tlp_q[$];
                                   ep_one_tlp_q = ep_tlp_burst_q.pop_front();

                                   foreach (ep_one_tlp_q[i])
                                        `uvm_info("PIPE_RX_DRV", $sformatf(
                                                  "EP_SCRAMBLED_BEFORE %0h", ep_one_tlp_q[i]),
                                                  UVM_HIGH)

                                   // NOTE: this TLP already carries its own STP - added per-TLP in
                                   // write_port_g() before being pushed into ep_tlp_burst_q. Do not
                                   // re-run STP insertion here.
                                   foreach (ep_one_tlp_q[i])
                                        `uvm_info("PIPE_RX_DRV", $sformatf(
                                                  "EP_SCRAMBLED %0h", ep_one_tlp_q[i]), UVM_HIGH)
                                   ep_one_tlp_q.push_back(EDS);

                                   ep_byte_striping(ep_one_tlp_q);
     ...
     task rc_state_recovery_rcvrlock();
          ...
          if (rc_active_gen < rc_negotiated_gen) rc_directed_speed_change = 1'b1;

          `uvm_info(
              "TX_LTSSM",
              $sformatf(
                  "Recovery.RcvrLock - relocking @ Gen%0d (negotiated Gen%0d, directed_speed_change=%0b)",
          ...
          fork
               ...
               begin
                    repeat (8) begin
                         ...
                         rc_drive_os(tag_ts1(
                                     rc_TS1,
                                     rc_directed_speed_change,
                                     rc_negotiated_link_num,
                                     rc_negotiated_lane_num
                                     ));
                    end
                    @(posedge rc_pvif.PCLK);
                    rc_tx_bcast_valid(0);
               ...
               begin
                    rc_ltssm_timer(
                        cfg.recovery_rcvrlock_timeout);  // TODO: add 24ms timeout param to cfg
                    timeout_occured = 1;
                    disable fork;
               end
          join
          ...
          if (barrier_met) begin
               `uvm_info("TX_LTSSM", "Recovery.RcvrLock done (fast match) -> Recovery.RcvrCfg",
                         UVM_HIGH)
          ...
          if (!rc_changed_speed_recovery && rc_active_gen > 1) begin
               `uvm_info(
                   "TX_LTSSM",
                   "24ms timeout, no speed change yet this Recovery pass, rate>2.5GT/s -> Recovery.Speed (revert to Gen1/2.5GT/s)",
                   UVM_HIGH)
               rc_target_gen = 1;
          ...
          if (rc_changed_speed_recovery) begin
               `uvm_info(
                   "TX_LTSSM",
                   $sformatf(
                       "24ms timeout, already changed speed this Recovery pass -> Recovery.Speed (revert to Gen%0d, entry speed)",
          ...
          `uvm_info("TX_LTSSM", "24ms timeout, no conditions met -> DETECT_QUIET", UVM_HIGH)
          rc_state = DETECT_QUIET;
     endtask
     ...
     task rc_state_recovery_rcvrcfg();
          ...
          `uvm_info("TX_LTSSM", $sformatf("Recovery.RcvrCfg - advertising TS2s (speed_change=%0b)",
                                          rc_directed_speed_change), UVM_LOW)
          ...
          fork
               ...
               begin
                    ...
                    if (rc_directed_speed_change) path_to_speed = 1;
                    else path_to_idle = 1;
                    disable fork;
               end
               ...
               begin
                    rc_ltssm_timer(
                        cfg.recovery_rcvrcfg_timeout);  // TODO: add 48ms timeout param to cfg
                    timeout_occured = 1;
                    disable fork;
               end
          join
          ...
          if (path_to_speed) begin
               ...
               `uvm_info("TX_LTSSM",
                         $sformatf(
                             "Recovery.RcvrCfg -> Recovery.Speed (successful, Gen%0d -> Gen%0d)",
          ...
          if (path_to_idle) begin
               ...
               `uvm_info("TX_LTSSM", "Recovery.RcvrCfg -> Recovery.Idle", UVM_HIGH)
          ...
          // 48ms timeout branch
          if (cfg.gen == 1 || cfg.gen == 2) begin
               `uvm_info("TX_LTSSM", "48ms timeout at Gen1/Gen2 -> DETECT_QUIET", UVM_HIGH)
               rc_state = DETECT_QUIET;
          end else if (cfg.gen == 3 && rc_idle_to_rlock_transitioned < 8'hFF) begin
               `uvm_info("TX_LTSSM",
                         "48ms timeout, Gen3, idle_to_rlock_transitioned<FFh -> Recovery.Idle",
                         UVM_HIGH)
               rc_changed_speed_recovery = 1'b0;
               rc_directed_speed_change = 1'b0;
               ...
               `uvm_info("TX_LTSSM", "48ms timeout, fallback -> DETECT_QUIET", UVM_HIGH)
               rc_state = DETECT_QUIET;
          end
     endtask
     ...
     task rc_state_recovery_speed();
          ...
          if (rc_successful_speed_negotiation) begin
               ...
               `uvm_info("TX_LTSSM", $sformatf(
                         "Recovery.Speed: successful negotiation -> Gen%0d", rc_active_gen),
                         UVM_LOW);

          end else if (rc_changed_speed_recovery) begin

               rc_active_gen = rc_gen_on_recovery_entry;
               rc_changed_speed_recovery = 1'b0;

               `uvm_info("TX_LTSSM", $sformatf(
                         "Recovery.Speed: reverting to entry Gen%0d", rc_active_gen), UVM_LOW);

          end else begin

               rc_active_gen = 1;

               `uvm_info("TX_LTSSM", "Recovery.Speed: falling back to Gen1", UVM_HIGH);
          ...
          `uvm_info("TX_LTSSM", $sformatf("Recovery.Speed: Rate changed to Gen%0d", rc_active_gen),
                    UVM_LOW);
          ...
          if (timeout_occurred && !speed_done) begin
               ...
               `uvm_info("TX_LTSSM", "48ms timeout in Recovery.Speed (abnormal) -> DETECT_QUIET",
                         UVM_HIGH);
          ...
          if (speed_done) begin
               ...
               `uvm_info(
                   "TX_LTSSM", $sformatf(
                   "Recovery.Speed done -> now running Gen%0d -> Recovery.RcvrLock", rc_active_gen),
                   UVM_LOW);
     ...
     task rc_state_recovery_idle();
          ...
          `uvm_info("TX_LTSSM", $sformatf(
                                    "Entered Recovery.Idle @ Gen%0d (via_rcvrcfg_timeout=%0b)",
          ...
          if (cfg.link_ctrl_hot_reset) begin
               `uvm_info("TX_LTSSM", "Hot Reset directed -> immediate HOT_RESET", UVM_HIGH)
               rc_state = HOT_RESET;
               return;
          end
          ...
          fork
               ...
               begin
                    rc_ltssm_timer(
                        cfg.recovery_idle_timeout);  // TODO: add 2ms timeout param to cfg
                    timeout_occured = 1;
                    disable fork;
               end
          join
          ...
          if (l0_ready) begin
               `uvm_info("TX_LTSSM", "Recovery.Idle: 8x consecutive Idle -> L0", UVM_HIGH)
               rc_idle_to_rlock_transitioned = 8'h00;  // reset on transition to L0
               rc_state = L0;
               return;
          end
          ...
          if (config_ready) begin
               `uvm_info("TX_LTSSM", "Recovery.Idle: PAD-Lane TS1 detected -> Configuration",
                         UVM_HIGH)
               rc_state = CONFIG_LINKNUM_START;
               return;
          end
          ...
          if (rc_idle_to_rlock_transitioned < 8'hFF) begin
               `uvm_info("TX_LTSSM",
                         $sformatf(
                             "2ms timeout, idle_to_rlock_transitioned=%0h<FFh -> Recovery.RcvrLock",
                             rc_idle_to_rlock_transitioned), UVM_LOW)
               if (cfg.gen == 3)
                    rc_idle_to_rlock_transitioned = rc_idle_to_rlock_transitioned + 1'b1;
     ...
     task ep_state_recovery_rcvrlock();
          ...
          if (ep_active_gen < ep_negotiated_gen) ep_directed_speed_change = 1'b1;
          ...
          fork
               begin
                    repeat (8) begin
                         ...
                         ep_drive_os(tag_ts1(
                                     ep_TS1,
                                     ep_directed_speed_change,
                                     ep_negotiated_link_num,
                                     ep_negotiated_lane_num
                                     ));
                    end
                    @(posedge ep_pvif.PCLK);
                    ep_tx_bcast_valid(0);
          ...
          if (barrier_met) begin
               `uvm_info("RX_LTSSM", "Recovery.RcvrLock done (fast match) -> Recovery.RcvrCfg",
                         UVM_HIGH)
          ...
          `uvm_info("RX_LTSSM", "24ms timeout, no conditions met -> DETECT_QUIET", UVM_HIGH)
          ep_state = EP_DETECT_QUIET;
     endtask
     ...
     task ep_state_recovery_rcvrcfg();
          ...
          `uvm_info("RX_LTSSM", $sformatf("Recovery.RcvrCfg - advertising TS2s (speed_change=%0b)",
                                          ep_directed_speed_change), UVM_LOW)
          ...
          fork
               ...
               begin
                    ...
                    if (ep_directed_speed_change) path_to_speed = 1;
                    else path_to_idle = 1;
                    disable fork;
               end
          ...
          if (path_to_idle) begin
               ...
               `uvm_info("RX_LTSSM", "Recovery.RcvrCfg -> Recovery.Idle", UVM_HIGH)
               ep_state = EP_RECOVERY_IDLE;
               return;
          end

          if (cfg.gen == 1 || cfg.gen == 2) begin
               `uvm_info("RX_LTSSM", "48ms timeout at Gen1/Gen2 -> DETECT_QUIET", UVM_HIGH)
               ep_state = EP_DETECT_QUIET;
          end else if (cfg.gen == 3 && ep_idle_to_rlock_transitioned < 8'hFF) begin
               `uvm_info("RX_LTSSM",
                         "48ms timeout, Gen3, idle_to_rlock_transitioned<FFh -> Recovery.Idle",
               ...
               `uvm_info("RX_LTSSM", "48ms timeout, fallback -> DETECT_QUIET", UVM_HIGH)
               ep_state = EP_DETECT_QUIET;
          end
     endtask
     ...
     task ep_state_recovery_speed();
          ...
          if (ep_successful_speed_negotiation) begin
               ...
               `uvm_info("RX_LTSSM", $sformatf(
                         "Recovery.Speed: successful negotiation -> Gen%0d", ep_active_gen),
                         UVM_LOW);

          end else if (ep_changed_speed_recovery) begin

               ep_active_gen = ep_gen_on_recovery_entry;
               ep_changed_speed_recovery = 1'b0;

               `uvm_info("RX_LTSSM", $sformatf(
                         "Recovery.Speed: reverting to entry Gen%0d", ep_active_gen), UVM_LOW);

          end else begin

               ep_active_gen = 1;

               `uvm_info("RX_LTSSM", "Recovery.Speed: falling back to Gen1", UVM_HIGH);
          ...
          `uvm_info("RX_LTSSM", $sformatf("Recovery.Speed: Rate changed to Gen%0d", ep_active_gen),
                    UVM_LOW);
          ...
          if (timeout_occurred && !speed_done) begin
               ...
               `uvm_info("RX_LTSSM", "48ms timeout in Recovery.Speed (abnormal) -> DETECT_QUIET",
                         UVM_HIGH);
          ...
          if (speed_done) begin
               ...
               `uvm_info(
                   "RX_LTSSM", $sformatf(
                   "Recovery.Speed done -> now running Gen%0d -> Recovery.RcvrLock", ep_active_gen),
                   UVM_LOW);
     ...
     task ep_state_recovery_idle();
          ...
          if (cfg.link_ctrl_hot_reset) begin
               `uvm_info("RX_LTSSM", "Hot Reset TS1s received (modeled) -> immediate EP_HOT_RESET",
                         UVM_HIGH)
          ...
          if (l0_ready) begin
               `uvm_info("RX_LTSSM", "Recovery.Idle: 8x consecutive Idle -> L0", UVM_HIGH)
               ep_idle_to_rlock_transitioned = 8'h00;
               ep_state = EP_L0;
               return;
          end
          ...
          if (config_ready) begin
               `uvm_info("RX_LTSSM", "Recovery.Idle: PAD-Lane TS1 detected -> Configuration",
                         UVM_HIGH)
               ep_state = EP_CONFIG_LINKNUM_START;
               return;
          end
     ...
     task rc_state_detect_quiet();
          ...
          `uvm_info("TX_LTSSM", "Entered DETECT_QUIET", UVM_HIGH)
          ...
          `uvm_info("TX_LTSSM", "Exit DETECT_QUIET -> DETECT_ACTIVE", UVM_HIGH)
     ...
     task ep_state_detect_quiet();
          ...
          `uvm_info("RX_LTSSM", "Entered DETECT_QUIET", UVM_HIGH)
          ...
          `uvm_info("RX_LTSSM", "Exit DETECT_QUIET -> DETECT_ACTIVE", UVM_HIGH)
     ...
     task rc_state_disabled();
          ...
          `uvm_info("TX_LTSSM", "Entered DISABLED - broadcasting TS1 with Disable Link asserted",
                    UVM_HIGH)
          ...
          `uvm_info("TX_LTSSM",
                    $sformatf(
                        "DISABLED: EIOS transmitted, EIOS received=%0b -> LinkUp=0, Lanes Disabled",
                        rx_eios_seen), UVM_LOW)
     ...
     task ep_state_disabled();
          ...
          `uvm_info("RX_LTSSM", "Entered DISABLED - broadcasting TS1 with Disable Link asserted",
                    UVM_HIGH)
     ...
     task rc_state_loopback_entry();
          ...
          `uvm_info("TX_LTSSM",
                    "Entered LOOPBACK_ENTRY (master) - broadcasting TS1 with Loopback asserted",
                    UVM_HIGH)
          ...
          if (ready) begin
               `uvm_info("TX_LTSSM",
                         "LOOPBACK_ENTRY: slave TS1s w/ Loopback seen -> LOOPBACK_ACTIVE", UVM_HIGH)
               rc_state = LOOPBACK_ACTIVE;
               return;
          end

          `uvm_info("TX_LTSSM", "LOOPBACK_ENTRY: timeout (<100ms bound) -> LOOPBACK_EXIT", UVM_HIGH)
     ...
     task rc_state_loopback_active();
          ...
          `uvm_info("TX_LTSSM", "Entered LOOPBACK_ACTIVE (master) - waiting for directed exit",
                    UVM_HIGH)
          ...
          `uvm_info("TX_LTSSM", $sformatf(
                                    "LOOPBACK_ACTIVE: directed=%0b timeout=%0b -> LOOPBACK_EXIT",
                                    directed, timeout_occured), UVM_LOW)
          rc_state = LOOPBACK_EXIT;
     ...
     task rc_state_loopback_exit();
          ...
          `uvm_info("TX_LTSSM", "Entered LOOPBACK_EXIT (master) - EIOS burst then Electrical Idle",
                    UVM_HIGH)
          ...
          `uvm_info("TX_LTSSM",
                    "LOOPBACK_EXIT: 2ms Electrical Idle complete -> LinkUp=0, exiting Loopback",
                    UVM_HIGH)
     ...
     task ep_state_loopback_entry();
          ...
          `uvm_info("RX_LTSSM", "Entered LOOPBACK_ENTRY (slave) - broadcasting PAD/PAD TS1",
                    UVM_HIGH)
          ...
          if (ready) begin
               `uvm_info("RX_LTSSM", "LOOPBACK_ENTRY: Symbol lock (approx) -> LOOPBACK_ACTIVE",
                         UVM_HIGH)
               ep_state = EP_LOOPBACK_ACTIVE;
               return;
          end

          `uvm_info("RX_LTSSM", "LOOPBACK_ENTRY: timeout -> LOOPBACK_EXIT", UVM_HIGH)
     ...
     task ep_state_loopback_active();
          ...
          `uvm_info("RX_LTSSM",
                    "Entered LOOPBACK_ACTIVE (slave) - watching for directed exit or EIOS",
                    UVM_HIGH)
          ...
          `uvm_info("RX_LTSSM",
                    $sformatf(
                        "LOOPBACK_ACTIVE: directed=%0b eios_seen=%0b timeout=%0b -> LOOPBACK_EXIT",
                        directed, eios_seen, timeout_occured), UVM_LOW)
          ep_state = EP_LOOPBACK_EXIT;
     ...
     task ep_state_loopback_exit();
          ...
          `uvm_info("RX_LTSSM", "Entered LOOPBACK_EXIT (slave) - Electrical Idle", UVM_HIGH)
     ...
     task rc_state_hot_reset(output bit remain);
          ...
          `uvm_info("TX_LTSSM",
                    "Entered HOT_RESET (directed) - broadcasting TS1 with Hot Reset asserted",
                    UVM_HIGH)
          ...
          fork
               begin
                    repeat (cfg.hot_reset_ts1_count) begin
                         ...
                         rc_drive_os(tag_ts1(
                                     rc_TS1,
                                     .link_num(rc_link_num),
                                     .lane_num(rc_lane_num),
                                     .hot_reset(1'b1)
                                     ));
                    end
                    @(posedge rc_pvif.PCLK);
          ...
          if (echo_seen && cfg.hot_reset_remain_directed) begin
               `uvm_info(
                   "TX_LTSSM",
                   "HOT_RESET: echo seen, higher Layer still directing -> remaining in HOT_RESET",
                   UVM_HIGH)
               remain = 1;
               return;
          end
          ...
          `uvm_info("TX_LTSSM",
                    $sformatf("HOT_RESET: echo_seen=%0b timeout=%0b -> LinkUp=0, exiting Hot Reset",
                              echo_seen, timeout_occured), UVM_LOW)
          remain = 0;
     ...
     task ep_state_hot_reset(output bit remain);
          ...
          `uvm_info("RX_LTSSM",
                    "Entered HOT_RESET (not directed) - broadcasting TS1 with Hot Reset asserted",
                    UVM_HIGH)
          ...
          fork
               begin
                    repeat (cfg.hot_reset_ts1_count) begin
                         ...
                         ep_drive_os(tag_ts1(
                                     ep_TS1,
                                     .link_num(ep_link_num),
                                     .lane_num(ep_lane_num),
                                     .hot_reset(1'b1)
                                     ));
                    end
                    @(posedge ep_pvif.PCLK);
          ...
          if (echo_seen) begin
               `uvm_info(
                   "RX_LTSSM",
                   "HOT_RESET: matching TS1s still arriving -> remaining in HOT_RESET, timer reset",
                   UVM_HIGH)
               remain = 1;
               return;
          end

          `uvm_info("RX_LTSSM", $sformatf("HOT_RESET: timeout=%0b -> LinkUp=0, exiting Hot Reset",
                                          timeout_occured), UVM_LOW)
          remain = 0;
               ...
               RC_MODE: begin
                    ...
                    forever begin
                         case (rc_state)
                              ...
                              DISABLED: begin
                                   rc_state_disabled();
                                   `uvm_info("TX_LTSSM", "DISABLED complete - ending testcase",
                                             UVM_HIGH)
                                   return;
                              end
                              ...
                              LOOPBACK_EXIT: begin
                                   ...
                                   `uvm_info("TX_LTSSM", "LOOPBACK_EXIT complete - ending testcase",
                                             UVM_HIGH)
                                   return;
                              end
                              HOT_RESET: begin
                                   ...
                                   if (!remain) begin
                                        `uvm_info("TX_LTSSM",
                                                  "HOT_RESET complete - ending testcase", UVM_HIGH)
                                        return;
                                   end
                              end
               ...
               EP_MODE: begin
                    ...
                    forever begin
                         case (ep_state)
                              ...
                              EP_DISABLED: begin
                                   ep_state_disabled();
                                   `uvm_info("RX_LTSSM", "DISABLED complete - ending testcase",
                                             UVM_HIGH)
                                   return;
                              end
                              ...
                              EP_LOOPBACK_EXIT: begin
                                   ...
                                   `uvm_info("RX_LTSSM", "LOOPBACK_EXIT complete - ending testcase",
                                             UVM_HIGH)
                                   return;
                              end
                              EP_HOT_RESET: begin
