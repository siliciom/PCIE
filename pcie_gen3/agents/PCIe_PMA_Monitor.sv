class PCIe_PMA_monitor extends uvm_monitor;
  ...
  function void build_phase(uvm_phase phase);
    ...
    tag = get_full_name();
    `uvm_info("PCIe_PMA_monitor", $sformatf("[%s] Configured: mode=%s", tag, cfg.mode.name()), UVM_HIGH)
    ...
    case(cfg.mode)
      ...
      RC_MODE: begin
        ...
        `uvm_info("PCIe_PMA_monitor", $sformatf("[%s] RC interfaces connected", tag), UVM_HIGH)
      end
      ...
      EP_MODE: begin
        ...
        `uvm_info("PCIe_PMA_monitor", $sformatf("[%s] EP interfaces connected", tag), UVM_HIGH)
      end
        ...
        fork
          ...
          begin
            forever begin
                  ...
                  fork
                    begin
                      ...
                      while (1) begin
                        ...
                        pma_tx_item.tx_data_q[l].push_back(pipe_tx_vif.TxData[l]);
                      end
                      lanes_pending--;
                    end
                  join_none
              ...
              `uvm_info("PMA_TX_MON",
                $sformatf("[%s] TX packet captured on PMA lanes, forwarded to PMA RX path",
                           tag), UVM_LOW)
              pma_tx_rx_port.write(pma_tx_item);
              `uvm_info("PMA_TX_MON",
                $sformatf("[%s] MAC TX DATA CAPTURED lane0=%0d words time = %t",
                           tag, pma_tx_item.tx_data_q[0].size(), $time), UVM_HIGH)
            end
          end
          begin
            ...
            forever begin
              ...
              begin
                ...
                if(count_lane[lane] == 130) begin
                  ...
                  if (cur_item == null) begin
                    cur_item = Sequence_item::type_id::create("cur_item", this);
                    wait (Sequence_item::size_rx_tx.size() > 0);
                    expected_size = Sequence_item::size_rx_tx.pop_front();
                    `uvm_info("PMA_RX_MON", $sformatf("[%s] RC EXECEPTED_SIZE = %d time = %t",
              ...
              if(cur_item != null) begin
                ...
                if(all_lanes_done) begin
                  `uvm_info("PMA_TX_MON", $sformatf("[%s] size_rx = %d time = %t", tag, expected_size, $time), UVM_HIGH)
                  `uvm_info("PMA_TX_MON",
                    $sformatf("[%s] TX packet reassembled from PMA lanes, forwarded to MAC layer",
                               tag), UVM_LOW)
                  pma_tx_mac_port.write(cur_item);
                  `uvm_info("PMA_TX_MON", $sformatf("[%s] RC PMA RX DATA CAPTURED lane0=%p complete: %0d words time %t",
                                                     tag, cur_item.pma_tx_data[0], cur_item.pma_tx_data[0].size(), $time), UVM_HIGH)
                  cur_item = null;
                end
        ...
        fork
          ...
          begin
            forever begin
                  ...
                  fork
                    begin
                      ...
                      while (1) begin
                        ...
                        pma_rx_item.pma_tx_data_t[l].push_back(pipe_rx_vif.TxData[l]);
                      end
                      lanes_pending--;
                    end
                  join_none
              ...
              `uvm_info("PMA_RX_MON",
                $sformatf("[%s] EP PMA TX DATA CAPTURED lane0=%0p time = %t",
                           tag, pma_rx_item.pma_tx_data_t[0], $time), UVM_HIGH)
              `uvm_info("PMA_RX_MON",
                $sformatf("[%s] RX packet captured on PMA lanes, forwarded to PMA TX path", tag),
                UVM_LOW)
            ...
            forever begin
              ...
              begin
                ...
                if(count_lane[lane] == 130) begin
                  ...
                  if (cur_item == null) begin
                    cur_item = Sequence_item::type_id::create("pma_rx_capture_item", this);
                    wait (Sequence_item::size_tx_rx.size() > 0);
                    expected_size = Sequence_item::size_tx_rx.pop_front();
                    `uvm_info("PMA_RX_MON", $sformatf("[%s] EP EXECEPTED_SIZE = %d time = %t",
                                                       tag,expected_size, $time), UVM_HIGH)
                  end
              ...
              if(cur_item != null) begin
                ...
                if(all_lanes_done) begin
                  `uvm_info("PMA_RX_MON",
                    $sformatf("[%s] RX packet reassembled from PMA lanes, forwarded to MAC layer",
                               tag), UVM_LOW)
                  pma_rx_mac_port.write(cur_item);
                  `uvm_info("PMA_RX_MON", $sformatf("[%s] EP PMA RX DATA lane0=%0p words time = %t",
