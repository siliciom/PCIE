...
class PCIe_DLL_Driver extends uvm_driver #(Sequence_item);
          ...
          case (cfg.mode)
               ...
               RC_MODE: begin
                    ...
                    if (!uvm_config_db#(virtual TX_TL_DL_Interface)::get(
                         ...
                         `uvm_fatal("PCIe_DLL_Driver", $sformatf(
                                    "[%s] Unable To Access TX_TL_DL", tag))

                    if (!uvm_config_db#(virtual TX_DLL_PCS_Interface)::get(
                            this, "", "DLL_Vif", TX_DLL_PCS
                        ))
                         `uvm_fatal("PCIe_DLL_Driver", $sformatf(
                                    "[%s] Unable To Access TX_DLL_PCS", tag))
               ...
               EP_MODE: begin
                    ...
                    if (!uvm_config_db#(virtual RX_DLL_PCS_Interface)::get(
                            this, "", "DLL_Vif", RX_DLL_PCS
                        ))
                         `uvm_fatal("RX_DLL_DRIVER", $sformatf(
                                    "[%s] Unable To Access RX_DLL_PCS", tag))
     ...
     function void write_fc(Sequence_item t_x);
          ...
          if (t_x.ep_updated_credits == 1) begin
               ...
               `uvm_info(
                   "WR_EP_FC_UPDATE",
                   $sformatf(
                       "EP FC Credits: VC=%0d, PH=%0h, PD=%0d, NPH=%0h, NPD=%0h, CPLH=%0h, CPLD=%0h",
                       t_x.ep_vc, t_x.ep_fc_ph, t_x.ep_fc_pd, t_x.ep_fc_nph, t_x.ep_fc_npd,
                       t_x.ep_fc_cmplh, t_x.ep_fc_cmpld), UVM_LOW)
     ...
     function bit [15:0] crc16(input bit [31:0] data);
     ...
     function bit [47:0] create_dllp(dllp_type_e dllp_type, bit [2:0] vc_id,
                                     bit [7:0] header_credit, bit [11:0] data_credit);
          bit [47:0] dllp_pkt;
          bit [15:0] dllp_crc;
     ...
     function bit [31:0] rc_calculate_lcrc(input bit [31:0] pkt_q[$]);
          ...
          foreach (pkt_q[i]) begin
               for (int b = 0; b < 32; b++) begin
                    ...
                    crc = crc >> 1;
                    if (feedback) crc ^= 32'hEDB8_8320;  // Standard Polynomial - 04C11DB7
     ...
     function bit [47:0] ack_nak_dllp(ack_nak_e ack_nak_type, bit [11:0] ack_nak_seq);
     ...
     task rc_ackd_seq(input bit [7:0] rc_dllp_type, input bit [11:0] rc_ack_nak_seq);
          begin
               ...
               if (rc_dllp_type == 8'b0000_0000) begin
                    ...
                    while (rc_rd_ptr != rc_wr_ptr) begin
                         ...
                         if (seq == rc_ack_nak_seq) begin
                              ...
                              $display(
                                  "[%0t] DELETE ACKED ENTRY : rd_ptr=%0d seq=%0d queue_size=%0d",
                                  $time, rc_rd_ptr, seq,
                              ...
                              break;
                         end  // Delete entries preceding the ACKed sequence
                         else begin

                              $display(
                                  "[%0t] DELETE PREVIOUS ENTRY : rd_ptr=%0d seq=%0d queue_size=%0d",
                                  $time, rc_rd_ptr, seq,
                                  rc_replay_buffer[rc_rd_ptr].packet_q.size());
     ...
     task rc_nak(input bit [7:0] rc_dllp_type, input bit [11:0] rc_ack_nak_seq);
          ...
          while (rc_replay_ptr != rc_wr_ptr) begin
               if (rc_replay_buffer[rc_replay_ptr].replay_num == 3) begin
                    ...
                    `uvm_error("REPLAY", $sformatf(
                                             "Seq %0d exceeded replay limit",
     ...
     task rc_replay_timer();
          ...
          forever begin
               ...
               if (rc_replay_timer_running) begin
                    rc_replay_timer_count++;

                    if (rc_replay_timer_count >= rc_replay_timer_limit) begin
     ...
     task rc_send_one_dllp(bit [47:0] dllp_pkt);
          ...
          `uvm_info("DLLP_TX", $sformatf("RC: Sending DLLP DATA[0] to TX_DLL_PCS.dl_tx_data = %08h",
                                         dllp_pkt[31:0]), UVM_LOW)

          wait (TX_DLL_PCS.dl_tx_ready);
          @(posedge TX_DLL_PCS.CLK);

          TX_DLL_PCS.dl_tx_data <= {dllp_pkt[47:32], 16'h0000};
          `uvm_info("DLLP_TX", $sformatf("RC: Sending DLLP DATA[1] to TX_DLL_PCS.dl_tx_data = %08h",
                                         {dllp_pkt[47:32], 16'h0000}), UVM_LOW)
     ...
     task rc_send_initfc1_dllps();
          ...
          for (int vc = 0; vc < `NUM_VC; vc++) begin
               ...
               initfc1_p   = create_dllp(INITFC1_P, vc[2:0], rc_fc_ph[vc], rc_fc_pd[vc]);

               initfc1_np  = create_dllp(INITFC1_NP, vc[2:0], rc_fc_nph[vc], rc_fc_npd[vc]);

               initfc1_cpl = create_dllp(INITFC1_CPL, vc[2:0], rc_fc_cmplh[vc], rc_fc_cmpld[vc]);
     ...
     task rc_send_initfc2_dllps();
          ...
          for (int vc = 0; vc < `NUM_VC; vc++) begin
               ...
               initfc2_p   = create_dllp(INITFC2_P, vc[2:0], rc_fc_ph[vc], rc_fc_pd[vc]);

               initfc2_np  = create_dllp(INITFC2_NP, vc[2:0], rc_fc_nph[vc], rc_fc_npd[vc]);

               initfc2_cpl = create_dllp(INITFC2_CPL, vc[2:0], rc_fc_cmplh[vc], rc_fc_cmpld[vc]);
     ...
     task rc_Updatefc_dllps();
          ...
          for (int vc = 0; vc < 1; vc++) begin
               ...
               updatefc_p   = create_dllp(UPDATEFC_P, vc[2:0], rc_fc_ph[vc], rc_fc_pd[vc]);

               updatefc_np  = create_dllp(UPDATEFC_NP, vc[2:0], rc_fc_nph[vc], rc_fc_npd[vc]);

               updatefc_cpl = create_dllp(UPDATEFC_CPL, vc[2:0], rc_fc_cmplh[vc], rc_fc_cmpld[vc]);
     ...
     task rc_rx_recived_packet(input bit [31:0] rx_packet_recived[$]);

          $display("[%0t][RX_DLL_DRIVER] Collect DATA = %p", $time, rx_packet_recived);
          ...
          foreach (rx_packet_recived[i]) begin
               ...
               $display("[%0t][RX_DLL_DRIVER] Sent DATA[%0d] = %h", $time, i, rx_packet_recived[i]);
     ...
     task rc_send_packet(input bit [31:0] rx_packet_send[$]);
          ...
          foreach (rx_packet_send[i]) begin
               ...
               `uvm_info(
                   "TLP_TX", $sformatf(
                   "RC: Sending DATA[%0d] to TX_DLL_PCS.dl_tx_data = %08h", i, rx_packet_send[i]),
                   UVM_LOW)
          ...
          rc_phy_tx_key.put(1);
          if (cfg.inject_err == ERR_STP) cfg.inject_err = ERR_NONE;

     endtask

     task rc_tx_recived_packet(input bit [31:0] tx_packet_recived[$]);
          ...
          $display("@(%0t) DLL_SEQ = %0h", $time, {20'd0, rc_DL_SEQ});
          ...
          $display("@(%0t) DLL_LCRC = %0h", $time, rx_lcrc);
          ...
          if (rc_outstanding_pkt_count == 1) begin
               rc_replay_timer_running = 1;
               rc_replay_timer_count   = 0;
          end
     ...
     task ep_ackd_seq(input bit [7:0] ep_dllp_type, input bit [11:0] ep_ack_nak_seq);
          begin
               ...
               if (ep_dllp_type == 8'b0000_0000) begin
                    ...
                    while (ep_rd_ptr != ep_wr_ptr) begin
                         ...
                         if (seq == ep_ack_nak_seq) begin
                              ...
                              $display(
                                  "[%0t] EP DELETE ACKED ENTRY : rd_ptr=%0d seq=%0d queue_size=%0d",
                                  $time, ep_rd_ptr, seq,
                              ...
                              $display(
                                  "[%0t] EP DELETE PREVIOUS ENTRY : rd_ptr=%0d seq=%0d queue_size=%0d",
                                  $time, ep_rd_ptr, seq,
                                  ep_replay_buffer[ep_rd_ptr].ep_packet_q.size());
     ...
     task ep_replay_timer();
          ...
          forever begin
               ...
               if (ep_replay_timer_running) begin
                    ep_replay_timer_count++;

                    if (ep_replay_timer_count >= ep_replay_timer_limit) begin
     ...
     task ep_send_one_dllp(bit [47:0] dllp_pkt);
          ...
          `uvm_info("DLLP_TX", $sformatf("EP: Sending DLLP DATA[0] to RX_DLL_PCS.dl_tx_data = %08h",
                                         dllp_pkt[31:0]), UVM_LOW)
          ...
          RX_DLL_PCS.dl_tx_data <= {dllp_pkt[47:32], 16'h0000};
          `uvm_info("DLLP_TX", $sformatf("EP: Sending DLLP DATA[1] to RX_DLL_PCS.dl_tx_data = %08h",
                                         {dllp_pkt[47:32], 16'h0000}), UVM_LOW)
     ...
     task ep_send_initfc1_dllps();
          ...
          for (int vc = 0; vc < `NUM_VC; vc++) begin
               ...
               initfc1_p   = create_dllp(INITFC1_P, vc[2:0], ep_fc_ph[vc], ep_fc_pd[vc]);

               initfc1_np  = create_dllp(INITFC1_NP, vc[2:0], ep_fc_nph[vc], ep_fc_npd[vc]);

               initfc1_cpl = create_dllp(INITFC1_CPL, vc[2:0], ep_fc_cmplh[vc], ep_fc_cmpld[vc]);
     ...
     task ep_send_initfc2_dllps();
          ...
          for (int vc = 0; vc < `NUM_VC; vc++) begin
               ...
               initfc2_p   = create_dllp(INITFC2_P, vc[2:0], ep_fc_ph[vc], ep_fc_pd[vc]);

               initfc2_np  = create_dllp(INITFC2_NP, vc[2:0], ep_fc_nph[vc], ep_fc_npd[vc]);

               initfc2_cpl = create_dllp(INITFC2_CPL, vc[2:0], ep_fc_cmplh[vc], ep_fc_cmpld[vc]);
     ...
     task ep_Updatefc_dllps(input bit [2:0] vc);
          ...
          updatefc_p   = create_dllp(UPDATEFC_P, vc, ep_fc_ph[vc], ep_fc_pd[vc]);

          updatefc_np  = create_dllp(UPDATEFC_NP, vc, ep_fc_nph[vc], ep_fc_npd[vc]);

          updatefc_cpl = create_dllp(UPDATEFC_CPL, vc, ep_fc_cmplh[vc], ep_fc_cmpld[vc]);
     ...
     task ep_rx_recived_packet(input bit [31:0] rx_packet_recived[$]);
     ...
     task ep_send_packet(input bit [31:0] rx_packet_send[$]);
          ep_phy_tx_key.get(1);
          repeat (10) begin
               @(posedge RX_DLL_PCS.CLK);
               RX_DLL_PCS.dl_tx_valid <= 1'b0;
          ...
          foreach (rx_packet_send[i]) begin
               ...
               `uvm_info(
                   "TLP_TX", $sformatf(
                   "EP: Sending TLP DATA[%0d] to RX_DLL_PCS.dl_tx_data = %08h", i, rx_packet_send[i]
                   ), UVM_LOW)
          end
          @(posedge RX_DLL_PCS.CLK);
          RX_DLL_PCS.dl_tx_valid <= 1'b0;
     ...
     task ep_tx_recived_packet(input bit [31:0] tx_packet_recived[$]);
          ...
          $display("@(%0t) DLL_SEQ = %0h", $time, {20'd0, ep_RX_DL_SEQ});
          ...
          if (ep_outstanding_pkt_count == 1) begin
               ep_replay_timer_running = 1;
               ep_replay_timer_count   = 0;
          end
     endtask
                    ...
                    case (rc_dl_state)
                         ...
                         DL_INACTIVE: begin
                              ...
                              wait (TX_DLL_PCS.rc_link_up)
                                   `uvm_info("DLCMSM", "RC DLCMSM: DL_INACTIVE -> DL_INIT_FC1",
                                             UVM_HIGH)
                         ...
                         DL_INIT_FC1: begin
                              ...
                              while (rc_initfc1_rx_cnt < (3 * `NUM_VC)) begin
                                   ...
                                   if (rc_initfc1_timeout) begin
                                        `uvm_info(
                                            "DLCMSM",
                                            $sformatf(
                                                "RC DLCMSM: DL_INIT_FC1 retransmit (rx_cnt=%0d/%0d) - no full triplet set within 34us",
                                                rc_initfc1_rx_cnt, 3 * `NUM_VC), UVM_LOW)
                              ...
                              rc_fc_to_tl();
                              `uvm_info("DLCMSM", "RC DLCMSM: DL_INIT_FC1 -> DL_INIT_FC2", UVM_HIGH)
                         ...
                         DL_INIT_FC2: begin
                              ...
                              while (rc_initfc2_rx_cnt < (3 * `NUM_VC)) begin
                                   ...
                                   if (rc_initfc2_timeout) begin
                                        `uvm_info(
                                            "DLCMSM",
                                            $sformatf(
                                                "RC DLCMSM: DL_INIT_FC2 retransmit (rx_cnt=%0d/%0d) - no full triplet set within 34us",
                                                rc_initfc2_rx_cnt, 3 * `NUM_VC), UVM_LOW)
                                        rc_send_initfc2_dllps();
                                   end
                              ...
                              rc_dl_up = 1'b1;
                              `uvm_info("DLCMSM", "RC DLCMSM: DL_INIT_FC2 -> DL_ACTIVE", UVM_HIGH)
                         ...
                         DL_ACTIVE: begin
                              ...
                              fork
                                   ...
                                   forever begin
                                        ...
                                        $display("[%0t][RX_DLL_DRIVER] Collect DATA = %p", $time,
                                                 rc_rx_local_queue);
                    ...
                    case (ep_dl_state)
                         ...
                         EP_DL_INACTIVE: begin
                              ...
                              wait (RX_DLL_PCS.ep_link_up)
                                   `uvm_info("DLCMSM", "EP DLCMSM: DL_INACTIVE -> DL_INIT_FC1",
                                             UVM_HIGH)
                         ...
                         EP_DL_INIT_FC1: begin
                              ...
                              while (ep_initfc1_rx_cnt < (3 * `NUM_VC)) begin
                                   ...
                                   if (ep_initfc1_timeout) begin
                                        `uvm_info(
                                            "DLCMSM",
                                            $sformatf(
                                                "EP DLCMSM: EP_DL_INIT_FC1 retransmit (rx_cnt=%0d/%0d) - no full triplet set within 34us",
                                                ep_initfc1_rx_cnt, 3 * `NUM_VC), UVM_LOW)
                              ...
                              ep_fc_to_tl();
                              `uvm_info("DLCMSM", "EP DLCMSM: DL_INIT_FC1 -> DL_INIT_FC2", UVM_HIGH)
                         ...
                         EP_DL_INIT_FC2: begin
                              ...
                              while (ep_initfc2_rx_cnt < (3 * `NUM_VC)) begin
                                   ...
                                   if (ep_initfc2_timeout) begin
                                        `uvm_info(
                                            "DLCMSM",
                                            $sformatf(
                                                "EP DLCMSM: EP_DL_INIT_FC2 retransmit (rx_cnt=%0d/%0d) - no full triplet set within 34us",
                                                ep_initfc2_rx_cnt, 3 * `NUM_VC), UVM_LOW)
                                        ep_send_initfc2_dllps();
                                   end
                              ...
                              ep_dl_up = 1'b1;
                              `uvm_info("DLCMSM", "EP DLCMSM: DL_INIT_FC2 -> EP_DL_ACTIVE",
                                        UVM_HIGH)
