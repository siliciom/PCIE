class PCIe_DLL_Monitor extends uvm_monitor;
          ...
          case (cfg.mode)
               ...
               RC_MODE: begin
                    ...
                    if (!uvm_config_db#(virtual TX_TL_DL_Interface)::get(
                         ...
                         `uvm_fatal("PCIe_DLL_Monitor", $sformatf(
                                    "[%s] Unable to access TX_TL_DL from config_db", tag))

                    if (!uvm_config_db#(virtual TX_DLL_PCS_Interface)::get(
                            this, "", "DLL_Vif", TX_DLL_PCS
                        ))
                         `uvm_fatal("PCIe_DLL_Monitor", $sformatf(
                                    "[%s] Unable to access TX_DLL_PCS from config_db", tag))
               end
     ...
     task rc_monitor_rx();
          ...
          forever begin
               ...
               if (!(TX_DLL_PCS.dl_rx_valid && TX_DLL_PCS.dl_rx_ready)) continue;
     ...
     task rc_monitor_sb();
          ...
          forever begin
               ...
               // Wait until a valid transfer occurs
               if (!(TX_DLL_PCS.dl_tx_valid && TX_DLL_PCS.dl_tx_ready)) continue;
               ...
               if (TX_DLL_PCS.tl_packet) begin
                    ...
                    if (tx_pkt.tx_data_sb.size() > 0) begin
                         `uvm_info(
                             "DLL_MON_RC",
                             $sformatf(
                                 "RC DLL captured TX request on wire: %0d DW (seq+hdr+pyld+ecrc+lcrc), firstDW=%08h",
     ...
     task rc_collect_data_TX(Sequence_item t_x);
          ...
          while (1) begin
               ...
               if (TX_TL_DL.tl_tx_valid && TX_TL_DL.tl_tx_ready) begin
                    ...
                    `uvm_info("TX_COLLECT", $sformatf("RC: Collected DW = %08h (queue size=%0d)",
                                                      data_dw, t_x.tx_data_t.size()), UVM_LOW)
               end

               if (!TX_TL_DL.tl_tx_valid) break;
     ...
     task rc_collect_dllp(Sequence_item r_x);
          ...
          while (TX_DLL_PCS.dl_mac_packet) begin
               ...
               wait (TX_DLL_PCS.dl_rx_valid && TX_DLL_PCS.dl_rx_ready)
                    header[0] = TX_DLL_PCS.dl_rx_data;
               r_x.rc_dllp_data_sb.push_back(header[0]);

               `uvm_info("RX_PKT", $sformatf(
                         "RC: Received DATA on TX_DLL_PCS.dl_rx_data = %08h", header[0]), UVM_LOW)
               crc_pkt_q = header[0];
               @(negedge TX_DLL_PCS.CLK);
               wait (TX_DLL_PCS.dl_rx_valid && TX_DLL_PCS.dl_rx_ready)
               ...
               `uvm_info("RX_PKT", $sformatf(
                         "RC: Received DATA on TX_DLL_PCS.dl_rx_data = %08h", header[1]), UVM_LOW)
               crc = header[1][31:16];
               @(negedge TX_DLL_PCS.CLK);


               cal_crc = crc16(crc_pkt_q);
               `uvm_info("CRC_CHECK", $sformatf(
                         "RC: CRC MATCH - Calculated CRC = %04h, Received CRC = %04h", cal_crc, crc
                         ), UVM_LOW)
               `uvm_info("CRC_CHECK", $sformatf(
                         "EP: CRC MATCH - Calculated CRC = %04h, Received CRC = %04h", cal_crc, crc
                         ), UVM_LOW)
               if (cal_crc == crc) begin
                    ...
                    if (header[0][7:4] == 4'b0100 || header[0][7:4] == 4'b1000) begin
                         ...
                         `uvm_info(
                             "RC_FC",
                             $sformatf(
                                 "RC P FC DLLP: Type=%04b, VC=%0h, Header FC=%0h, Data FC=%0h",
                                 r_x.rc_data_type, r_x.rc_dllp_vc, r_x.rc_header_pfc,
                                 r_x.rc_data_pfc), UVM_LOW)
                    ...
                    if (header[0][7:4] == 4'b0101 || header[0][7:4] == 4'b1001) begin
                         ...
                         `uvm_info(
                             "RC_FC",
                             $sformatf(
                                 "RC NP FC DLLP: Type=%04b, VC=%0h, Header FC=%0h, Data FC=%0h",
                                 r_x.rc_data_type, r_x.rc_dllp_vc, r_x.rc_header_npfc,
                                 r_x.rc_data_npfc), UVM_LOW)
                    ...
                    if (header[0][7:4] == 4'b0110 || header[0][7:4] == 4'b1010) begin
                         ...
                         `uvm_info(
                             "RC_FC",
                             $sformatf(
                                 "RC CPL FC DLLP: Type=%04b, VC=%0h, Header FC=%0h, Data FC=%0h",
                                 r_x.rc_data_type, r_x.rc_dllp_vc, r_x.rc_header_cmplfc,
                                 r_x.rc_data_cmplfc), UVM_LOW)
                    ...
                    `uvm_info("CRC_CHECK", $sformatf(
                              "RC: CRC MISMATCH - Calculated CRC = %04h, Received CRC = %04h",
                              cal_crc,
                              crc
                              ), UVM_LOW)
                    `uvm_error(
                        "TX_DLL_MON", $sformatf(
                        "CRC MISMATCH : CALCULATED CRC = %08h RECEIVED CRC = %08h", cal_crc, crc))
               end
     ...
     task rc_collect_tlp(Sequence_item r_x);
          ...
          if (TX_DLL_PCS.tl_mac_packet) begin
               while (!(TX_DLL_PCS.dl_rx_valid && TX_DLL_PCS.dl_rx_ready));
               seq_no = TX_DLL_PCS.dl_rx_data;
               `uvm_info("RX_PKT", $sformatf("RC: Received DATA on TX_DLL_PCS.dl_rx_data = %08h",
                                             seq_no), UVM_LOW)
               lcrc_pkt_q.push_back(seq_no);
               r_x.rc_com_data_sb.push_back(seq_no);
               r_x.rc_seq_no = seq_no[11:0];

               `uvm_info("RC_SEQ_NO", $sformatf("RC Sequence Number assigned: %0d (0x%03h)",
                                                r_x.rc_seq_no, r_x.rc_seq_no), UVM_LOW)
               ////////////////////////////////////////////////////////////
               ...
               while (1) begin
                    ...
                    if (TX_DLL_PCS.dl_rx_valid && TX_DLL_PCS.dl_rx_ready) begin
                         ...
                         `uvm_info("RX_PKT",
                                   $sformatf("RC: Received DATA on TX_DLL_PCS.dl_rx_data = %08h",
                                             dw), UVM_LOW)
                         body_q.push_back(dw);
                    end
                    if (!TX_DLL_PCS.dl_rx_valid) break;
               end
               ////////////////////////////////////////////////////////////
               ...
               `uvm_info(
                   "DLL_MON_RC",
                   $sformatf(
                       "RC DLL collected RX completion: %0d DW (seq=%08h, hdr+pyld+ecrc=%0d DW, lcrc=%08h)",
               ...
               ////////////////////////////////////////////////////////////
               `uvm_info("LCRC_CHECK",
                         $sformatf("RC: LCRC MATCH - Calculated LCRC = %08h, Received LCRC = %08h",
                                   calc_lcrc, lcrc), UVM_LOW)
               `uvm_info("LCRC_CHECK",
                         $sformatf("EP: LCRC MATCH - Calculated LCRC = %08h, Received LCRC = %08h",
                                   calc_lcrc, lcrc), UVM_LOW)
               if (calc_lcrc == lcrc) begin
                    if (seq_no == RC_NRS) begin
                         rc_ack_ev.trigger();
                         ...
                         //////////////////////////////////////////////////////////
                         //////////////////////////////////////////////////////////
                         foreach (body_q[i]) r_x.rx_data_t.push_back(body_q[i]);
                    end else if (seq_no < RC_NRS) begin
                         rc_ack_ev.trigger();
                         r_x.rc_ack_nak_seq = seq_no;
                         RC_NACK_SCHEDULED  = 0;
                    end else if (seq_no > RC_NRS) begin
                         if (RC_NACK_SCHEDULED == 0) begin
                              ...
                              `uvm_error(
                                  "SEQ_CHECK",
                                  $sformatf(
                                      "Out-of-order/skipped DL sequence number: expected %0d, got %0d",
                                      RC_NRS, seq_no))
                    ...
                    `uvm_info("LCRC_CHECK", $sformatf(
                              "RC: LCRC MISMATCH - Calculated LCRC = %08h, Received LCRC = %08h",
                              calc_lcrc,
                              lcrc
                              ), UVM_LOW)
                    `uvm_error("TX_DLL_MON", $sformatf(
                               "LCRC MISMATCH : CALCULATED LCRC = %08h RECEIVED LCRC = %08h",
                               calc_lcrc,
                               lcrc
                               ))
                    if (RC_NACK_SCHEDULED == 0) begin
     ...
     task rc_collect_request(Sequence_item t_x);
          ...
          ////////////////////////////////////////////////////////////
          if (TX_DLL_PCS.tl_packet) begin
               if (TX_DLL_PCS.dl_tx_valid && TX_DLL_PCS.dl_tx_ready) seq_no = TX_DLL_PCS.dl_tx_data;
               ////////////////////////////////////////////////////////////
               // HEADER DW0
               ////////////////////////////////////////////////////////////
               do begin
                    @(negedge TX_DLL_PCS.CLK);
               end while (!(TX_DLL_PCS.dl_tx_valid && TX_DLL_PCS.dl_tx_ready));
               ...
               while (1) begin
                    ...
                    if (!TX_DLL_PCS.dl_tx_valid) break;
               end
               // STORE SEQ_NO
               t_x.tx_data_sb.push_back(seq_no);
               // STORE HEADER DW0
               t_x.tx_data_sb.push_back(header0);
               foreach (body_q[i]) t_x.tx_data_sb.push_back(body_q[i]);
          end
     endtask
     task rc_collect_dllp_sb(Sequence_item t_x);
          ...
          while (TX_DLL_PCS.dl_packet) begin
               ...
               wait (TX_DLL_PCS.dl_tx_valid && TX_DLL_PCS.dl_tx_ready)
               ...
               `uvm_info("RC_DLLP_RX", $sformatf(
                         "RC Received DLLP Packet = {%08h, %08h}", header[1], header[0]), UVM_LOW)
          end
     ...
     task ep_monitor_rx();
          ...
          forever begin
               ...
               // Wait until a valid transfer occurs
               if (!(RX_DLL_PCS.dl_rx_valid && RX_DLL_PCS.dl_rx_ready)) continue;
     ...
     task ep_monitor_sb();
          ...
          forever begin
               ...
               // Wait until a valid transfer occurs
               if (!(RX_DLL_PCS.dl_tx_valid && RX_DLL_PCS.dl_tx_ready)) continue;
               ...
               end else if (RX_DLL_PCS.dl_packet) begin
          ...
          forever begin
               ...
               if (RX_TL_DL.ep_fc_update_valid) begin
                    ...
                    `uvm_info(
                        "EP_FC_UPDATE",
                        $sformatf(
                            "EP FC Update: PH=%0h, NPH=%0h, CPLH=%0h, PD=%0h, NPD=%0h, CPLD=%0h, VC=%0h",
                            fc_pkt.ep_fc_ph, fc_pkt.ep_fc_nph, fc_pkt.ep_fc_cmplh, fc_pkt.ep_fc_pd,
                            fc_pkt.ep_fc_npd, fc_pkt.ep_fc_cmpld, fc_pkt.ep_vc), UVM_LOW)
     ...
     task ep_collect_data_tx(Sequence_item t_x);
          ...
          wait (RX_TL_DL.tl_tx_valid && RX_TL_DL.tl_tx_ready);
          ////////////////////////////////////////////////////
     ...
     task ep_collect_dllp(Sequence_item r_x);
          ...
          while (RX_DLL_PCS.dl_mac_packet) begin
               ...
               wait (RX_DLL_PCS.dl_rx_valid && RX_DLL_PCS.dl_rx_ready)
                    header[0] = RX_DLL_PCS.dl_rx_data;
               r_x.ep_dllp_data_sb.push_back(header[0]);


               `uvm_info("RX_PKT", $sformatf(
                         "EP: Received DATA on RX_DLL_PCS.dl_rx_data = %08h", header[0]), UVM_LOW)
               crc_pkt_q = header[0];
               @(negedge RX_DLL_PCS.CLK);
               wait (RX_DLL_PCS.dl_rx_valid && RX_DLL_PCS.dl_rx_ready);
               ...
               `uvm_info("RX_PKT", $sformatf(
                         "EP: Received DATA on RX_DLL_PCS.dl_rx_data = %08h", header[1]), UVM_LOW)
               crc = header[1][31:16];
               @(negedge RX_DLL_PCS.CLK);
               ...
               if (cal_crc == crc) begin
                    ...
                    if (header[0][7:4] == 4'b0100 || header[0][7:4] == 4'b1000) begin
                         ...
                         `uvm_info("EP_FC",
                                   $sformatf(
                                       "EP P FC DLLP: Type=%0b, VC=%0h, Header FC=%0h, Data FC=%0h",
                                       r_x.ep_data_type, r_x.ep_dllp_vc, r_x.ep_header_pfc,
                                       r_x.ep_data_pfc), UVM_LOW)
                    end
                    if (header[0][7:4] == 4'b0101 || header[0][7:4] == 4'b1001) begin
                         r_x.ep_data_type = header[0][7:4];
                         r_x.ep_dllp_vc = header[0][2:0];
                         ...
                         `uvm_info(
                             "EP_FC",
                             $sformatf(
                                 "EP NP FC DLLP: Type=%0b, VC=%0h, Header FC=%0h, Data FC=%0h",
                                 r_x.ep_data_type, r_x.ep_dllp_vc, r_x.ep_header_npfc,
                                 r_x.ep_data_npfc), UVM_LOW)
                    end
                    if (header[0][7:4] == 4'b0110 || header[0][7:4] == 4'b1010) begin
                         r_x.ep_data_type = header[0][7:4];
                         r_x.ep_dllp_vc = header[0][2:0];
                         ...
                         `uvm_info(
                             "EP_FC",
                             $sformatf(
                                 "EP CPL FC DLLP: Type=%0b, VC=%0h, Header FC=%0h, Data FC=%0h",
                                 r_x.ep_data_type, r_x.ep_dllp_vc, r_x.ep_header_cmplfc,
                                 r_x.ep_data_cmplfc), UVM_LOW)
                    end
                    ...
                    `uvm_info("CRC_CHECK", $sformatf(
                              "EP: CRC MISMATCH - Calculated CRC = %04h, Received CRC = %04h",
                              cal_crc,
                              crc
                              ), UVM_LOW)
                    `uvm_error(
     ...
     task ep_collect_tlp(Sequence_item r_x);
          ...
          if (RX_DLL_PCS.tl_mac_packet) begin
               if (RX_DLL_PCS.dl_rx_valid && RX_DLL_PCS.dl_rx_ready) seq_no = RX_DLL_PCS.dl_rx_data;
               `uvm_info("RX_PKT", $sformatf("EP: Received DATA on RX_DLL_PCS.dl_rx_data = %08h",
                                             seq_no), UVM_LOW)
               lcrc_pkt_q.push_back(seq_no);
               r_x.ep_req_data_sb.push_back(seq_no);
               r_x.ep_seq_no = seq_no[11:0];

               `uvm_info("EP_SEQ_NO", $sformatf("EP Sequence Number assigned: %0d (0x%03h)",
                                                r_x.ep_seq_no, r_x.ep_seq_no), UVM_LOW)
               ...
               while (1) begin
                    ...
                    if (RX_DLL_PCS.dl_rx_valid && RX_DLL_PCS.dl_rx_ready) begin
                         ...
                         `uvm_info("RX_PKT",
                                   $sformatf("EP: Received DATA on RX_DLL_PCS.dl_rx_data = %08h",
                                             dw), UVM_LOW)
                         body_q.push_back(dw);
                    end
                    if (!RX_DLL_PCS.dl_rx_valid) break;
               end
               lcrc = body_q.pop_back();
               foreach (body_q[i]) begin
                    lcrc_pkt_q.push_back(body_q[i]);
               ...
               `uvm_info(
                   "DLL_MON_EP",
                   $sformatf(
                       "EP DLL collected RX TLP: %0d DW (seq=%08h, hdr+pyld+ecrc=%0d DW, lcrc=%08h)",
               ...
               if (calc_lcrc == lcrc) begin
                    if (seq_no == EP_NRS) begin
                         ...
                         //////////////////////////////////////////////////////////
                         //////////////////////////////////////////////////////////
                         foreach (body_q[i]) r_x.rx_data.push_back(body_q[i]);
                    end else if (seq_no < EP_NRS) begin
                         ep_ack_ev.trigger();
                         r_x.ep_ack_nak_seq = seq_no;
                         EP_NACK_SCHEDULED  = 0;
                    end else if (seq_no > EP_NRS) begin
                    ...
                    `uvm_info("LCRC_CHECK", $sformatf(
                              "EP: LCRC MISMATCH - Calculated LCRC = %08h, Received LCRC = %08h",
                              calc_lcrc,
                              lcrc
                              ), UVM_LOW)
                    `uvm_error("RX_DLL_MON", $sformatf(
                               "LCRC MISMATCH : CALCULATED LCRC = %08h RECEIVED LCRC = %08h",
                               calc_lcrc,
                               lcrc
                               ))
     ...
     task ep_collect_completion(Sequence_item t_x);
          ...
          ////////////////////////////////////////////////////////////
          if (RX_DLL_PCS.tl_packet) begin
               if (RX_DLL_PCS.dl_tx_valid && RX_DLL_PCS.dl_tx_ready) seq_no = RX_DLL_PCS.dl_tx_data;
               ////////////////////////////////////////////////////////////
               // HEADER DW0
               ////////////////////////////////////////////////////////////
               do begin
                    @(negedge RX_DLL_PCS.CLK);
               end while (!(RX_DLL_PCS.dl_tx_valid && RX_DLL_PCS.dl_tx_ready));
               ...
               while (1) begin
                    ...
                    if (!RX_DLL_PCS.dl_tx_valid) break;
               end
               // STORE SEQ_NO
               t_x.rx_data_sb.push_back(seq_no);
               // STORE HEADER DW0
               t_x.rx_data_sb.push_back(header0);
               foreach (body_q[i]) t_x.rx_data_sb.push_back(body_q[i]);
          end
     endtask
     task ep_collect_dllp_sb(Sequence_item t_x);
          ...
          while (RX_DLL_PCS.dl_packet) begin
               ...
               wait (RX_DLL_PCS.dl_tx_valid && RX_DLL_PCS.dl_tx_ready)
               ...
               `uvm_info("RC_DLLP_RX", $sformatf(
                         "EP Received DLLP Packet = {%08h, %08h}", header[1], header[0]), UVM_LOW)
          end
