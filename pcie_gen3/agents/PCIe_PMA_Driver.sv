...
class PCIe_PMA_driver extends uvm_driver#(Sequence_item);
  ...
  function void build_phase(uvm_phase phase);
    ...
    tag = get_full_name();
    `uvm_info("PCIe_PMA_driver", $sformatf("[%s] Configured: mode=%s", tag, cfg.mode.name()), UVM_HIGH)
    ...
    case(cfg.mode)
      ...
      RC_MODE: begin
        ...
        `uvm_info("PCIe_PMA_driver", $sformatf("[%s] RC interfaces connected", tag), UVM_HIGH)
      end
      ...
      EP_MODE: begin
        ...
        `uvm_info("PCIe_PMA_driver", $sformatf("[%s] EP interfaces connected", tag), UVM_HIGH)
      end
  ...
  function void write_port_c(Sequence_item pma_rx);
    `uvm_info("PMA_TX_DRV", $sformatf("[%s] Driving TX packet onto PMA MAC lanes", tag), UVM_LOW)
    for(int lane = 0; lane < cfg.num_lanes; lane++)
      if(cfg.active_lane_mask[lane])
        foreach (pma_rx.pma_tx_data[lane][i])
          mac_tx_data_q[lane].push_back(pma_rx.pma_tx_data[lane][i]);
    `uvm_info("PMA_TX_DRV", $sformatf("[%s] mac_tx_data_q[0] size=%0d", tag, mac_tx_data_q[0].size()), UVM_HIGH)
  endfunction
  ...
  function void write_port_a(Sequence_item pma_rx);
    ...
    `uvm_info("PMA_RX_DRV", $sformatf("[%s] port_a: rx_data_q[0] now %0d entries", tag, rx_data_q[0].size()), UVM_HIGH)
  endfunction

  function void write_port_b(Sequence_item pma_rx);
    `uvm_info("PMA_RX_DRV", $sformatf("[%s] RX packet received on PMA MAC lanes", tag), UVM_LOW)
    ...
    `uvm_info("PMA_RX_DRV", $sformatf("[%s] port_b: mac_rx_data_q[0] %p now %0d entries", tag, mac_rx_data_q[0], mac_rx_data_q[0].size()), UVM_HIGH)
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    case(cfg.mode)
      ...
      RC_MODE: begin
        ...
        `uvm_info("PMA_TX", $sformatf("[%s] Ready for receiver detection (reactive, per-lane)", tag), UVM_HIGH)
          ...
          begin
            ...
            fork
              ...
              begin
                ...
                repeat(6) @(posedge pipe_tx_vif.PCLK);   // detection settle time
        ...
        fork
          begin
            forever begin
              ...
              `uvm_info("PMA_TX", $sformatf("[%s] RC Started TX DATA TRANSMISSION time = %t", tag, $time), UVM_HIGH)
              begin
              ...
              `uvm_info("PMA_TX", $sformatf("[%s] RC Completed TX DATA TRANSMISSION time = %t", tag, $time), UVM_HIGH)
            end
          end

          begin
            forever begin
              wait(mac_tx_data_q[0].size());   // representative lane 0 has data
              `uvm_info("PMA_TX", $sformatf("[%s] RC Started RX DATA TRANSMISSION time = %t", tag, $time), UVM_HIGH)
              size_t = mac_tx_data_q[0].size();
              ...
              for(int lane = 0; lane < cfg.num_lanes; lane++)
                if(cfg.active_lane_mask[lane])
                  pipe_tx_vif.RxValid[lane] <= 0;
              `uvm_info("PMA_TX", $sformatf("[%s] RC Stopped RX DATA TRANSMISSION time = %t", tag, $time), UVM_HIGH)
            end
          end
        join
      ...
      EP_MODE: begin
        ...
        `uvm_info("PMA_RX", $sformatf("[%s] Ready for receiver detection (reactive, per-lane)", tag), UVM_HIGH)
        ...
        fork
          begin
            forever begin
              ...
              `uvm_info("PMA_RX", $sformatf("[%s] EP Started TX DATA TRANSMISSION time = %t", tag, $time), UVM_HIGH)
              begin
                bit [129:0] tmp [`PCIE_NUM_LANES];
                int word_count;
              ...
              `uvm_info("PMA_RX", $sformatf("[%s] EP Stopped TX DATA TRANSMISSION time = %t", tag, $time), UVM_HIGH)
            end
          end

          begin
            forever begin
              wait(rx_data_q[0].size() > 0);   // representative lane 0 has data
              begin
                int burst_size;
                burst_size = rx_data_q[0].size();
                `uvm_info("PMA_RX", $sformatf("[%s] EP Started RX DATA TRANSMISSION time = %t", tag, $time), UVM_HIGH)
                ...
                `uvm_info("PMA_RX", $sformatf("[%s] EP Stopped RX DATA TRANSMISSION time = %t", tag, $time), UVM_HIGH)
              end
            end
          end
        join
