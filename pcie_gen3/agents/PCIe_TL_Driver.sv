class PCIe_TL_Driver extends uvm_driver #(Sequence_item);

  `uvm_component_utils(PCIe_TL_Driver)

  // This component's own role config (RC or EP)
  env_cfg cfg;

  // Only one of these is actually used, based on cfg.mode
  virtual TX_TL_DL_Interface TX_TL_DL;   // valid when RC_MODE
  virtual RX_TL_DL_Interface RX_TL_DL;   // valid when EP_MODE

  bit[31:0]  TLP_HEADER[0:3];
  bit[31:0]  TLP_PAYLOAD;
  bit[159:0] TLP_PACKET;

  string tag;

  function new(string name = "PCIe_TL_Driver", uvm_component parent = null);
    super.new(name, parent);
  endfunction

  function void build_phase(uvm_phase phase);
    super.build_phase(phase);

      if(!uvm_config_db#(env_cfg)::get(this, "", "env_cfg", cfg))
      `uvm_fatal("PCIe_TL_Driver",
        $sformatf("env_cfg not found for %s", get_full_name()))

    tag = get_full_name();

    `uvm_info("PCIe_TL_Driver",
      $sformatf("[%s] Configured: mode=%s", tag, cfg.mode.name()), UVM_LOW)

    //---------------------------------------------------
    // Get only the interface matching our own role.
    //---------------------------------------------------
    case(cfg.mode)

      RC_MODE: begin
        if(!uvm_config_db#(virtual TX_TL_DL_Interface)::get(this, "", "TL_Vif", TX_TL_DL))
          `uvm_fatal("PCIe_TL_Driver",
            $sformatf("[%s] Cannot get TX_TL_DL_Interface (TL_Vif)", tag))
        `uvm_info("RC_INTF_TL_DRV", $sformatf("[%s] TX_TL_DL_Interface connected", tag), UVM_LOW)
      end

      EP_MODE: begin
        if(!uvm_config_db#(virtual RX_TL_DL_Interface)::get(this, "", "TL_Vif", RX_TL_DL))
          `uvm_fatal("PCIe_TL_Driver",
            $sformatf("[%s] Cannot get RX_TL_DL_Interface (TL_Vif)", tag))
        `uvm_info("RC_INTF_TL_DRV", $sformatf("[%s] RX_TL_DL_Interface connected", tag), UVM_LOW)
      end

      default: `uvm_fatal("PCIe_TL_Driver", $sformatf("[%s] Unknown mode", tag))

    endcase

  endfunction

  task run_phase(uvm_phase phase);
    super.run_phase(phase);

    case(cfg.mode)

      RC_MODE: begin
        `uvm_info("PCIe_TL_Driver", $sformatf("[%s] RC driver active", tag), UVM_LOW)
              end

      EP_MODE: begin
        `uvm_info("PCIe_TL_Driver", $sformatf("[%s] EP driver active", tag), UVM_LOW)
              end

      default: `uvm_fatal("PCIe_TL_Driver", $sformatf("[%s] Unknown mode", tag))

    endcase

  endtask : run_phase

endclass : PCIe_TL_Driver
