...
class VC_Arbiter extends uvm_component;
     ...
     function void push(Sequence_item item);
          ...
          `uvm_info("VC_Arbiter", $sformatf(
                    "Enqueued item on VC%0d (queue depth=%0d)", item.vc, vc_q[item.vc].size()),
                    UVM_HIGH)
     endfunction
     ...
     // and its vc.
     task get_next(output Sequence_item item, output vc_id_e vc_won,
                   ref virtual TX_TL_DL_Interface TX_TL_DL);
               ...
               for (int v = 7; v >= 0; v--) begin
                    ...
                    if (vc_q[v].size() != 0) begin
                         ...
                         if (fc_mgr.has_sufficient_credit(vc_id_e'(v), vc_q[v][0])) begin
                              ...
                              `uvm_info("VC_Arbiter",
                                        $sformatf("Arbitration winner: VC%0d (strict priority)", v),
                                        UVM_LOW)
                              ...
                              `uvm_info(
                                  "VC_Arbiter", $sformatf(
                                  "VC%0d head-of-line blocked on credit, checking lower VCs", v),
                                  UVM_HIGH)
                         end
