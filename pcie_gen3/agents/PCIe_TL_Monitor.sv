...
class PCIe_TL_Monitor extends uvm_monitor;
     ...
     function void build_phase(uvm_phase phase);
          ...
          tag = get_full_name();
          `uvm_info("TX_TL_Monitor", $sformatf("[%s] Configured: mode=%s", tag, cfg.mode.name()),
                    UVM_LOW)
          ...
          case (cfg.mode)
               ...
               RC_MODE: begin
                    if (!uvm_config_db#(virtual TX_TL_DL_Interface)::get(
                         ...
                         `uvm_fatal("TX_TL_Monitor", $sformatf(
                                    "[%s] Unable to access TX_TL_DL interface", tag))
                    else
                         `uvm_info("TX_TL_Monitor", $sformatf(
                                   "[%s] Successfully accessed interface", tag), UVM_LOW)
               end
               ...
               EP_MODE: begin
                    if (!uvm_config_db#(virtual RX_TL_DL_Interface)::get(
                         ...
                         `uvm_fatal("RX_TL_MONITOR", $sformatf(
                                    "[%s] Unable to access RX TL interface", tag))
                    else
                         `uvm_info("RX_TL_Monitor", $sformatf(
                                   "[%s] Successfully accessed interface", tag), UVM_LOW)
               end
     ...
     task rc_sending_tx_request(Sequence_item t_x);
          ...
          case (t_x.r_type)
               5'b00000:
               if (t_x.fmt inside {3'b000, 3'b001}) t_x.e_type = MEM_RD;
               else t_x.e_type = MEM_WR;

               5'b00010:
               if (t_x.fmt == 3'b000) t_x.e_type = IO_RD;
               else t_x.e_type = IO_WR;

               5'b00100:
               if (t_x.fmt == 3'b000) t_x.e_type = CFG_RD0;
               else t_x.e_type = CFG_WR0;

               5'b00101:
               if (t_x.fmt == 3'b000) t_x.e_type = CFG_RD1;
               else t_x.e_type = CFG_WR1;

               5'b01010:
               if (t_x.fmt == 3'b000) t_x.e_type = CPL;
               else if (t_x.fmt == 3'b010) t_x.e_type = CPL_DATA;
          ...
          foreach (t_x.tlp_q[i])
               `uvm_info("RC_TL_MONITOR", $sformatf(
                         "HEADER %h header_dw = %d", t_x.tlp_q[i], header_dw), UVM_LOW)
          ...
          foreach (t_x.tlp_q[i])
               `uvm_info("RC_TL_MONITOR", $sformatf(
                         "HEADER data %h header_dw = %d", t_x.tlp_q[i], header_dw), UVM_LOW)

          `uvm_info("TX_TL_MONITOR", "=======================================================",
                    UVM_HIGH)
          foreach (t_x.tlp_q[i])
               `uvm_info(
                   "TX_TL_MONITOR", $sformatf(
                   "[%s] TL_TX_MONITOR_TLP_PACKET[%0d] = %0h @(%0t)", tag, i, t_x.tlp_q[i], $time),
                   UVM_LOW)

          `uvm_info("TX_TL_MONITOR",
                    $sformatf("[%s] DEBUG TX MONITOR - fmt=%0b r_type=%0b addr=%0h length=%0d",
     ...
     task rc_collecting_rx_completion(Sequence_item r_x);
          ...
          case (r_x.r_type)
               5'b00000:
               if (r_x.fmt inside {3'b000, 3'b001}) r_x.e_type = MEM_RD;
               else r_x.e_type = MEM_WR;

               5'b00010:
               if (r_x.fmt == 3'b000) r_x.e_type = IO_RD;
               else r_x.e_type = IO_WR;

               5'b00100:
               if (r_x.fmt == 3'b000) r_x.e_type = CFG_RD0;
               else r_x.e_type = CFG_WR0;

               5'b00101:
               if (r_x.fmt == 3'b000) r_x.e_type = CFG_RD1;
               else r_x.e_type = CFG_WR1;

               5'b01010:
               if (r_x.fmt == 3'b000) r_x.e_type = CPL;
               else if (r_x.fmt == 3'b010) r_x.e_type = CPL_DATA;
          ...
          `uvm_info("TX_TL_MONITOR", $sformatf("[%s] REQ_ID = %0d TAG = %0d BYTE_COUNT=%0d", tag,
                                               r_x.req_id, r_x.tag, r_x.byte_count), UVM_HIGH)
          ...
          if (r_x.td) begin
               ...
               `uvm_info("TX_TL_MONITOR", $sformatf(
                                              "[%s] RX ECRC CHECK: RECEIVED=%08h CALCULATED=%08h",
                                              tag, r_x.ECRC, calculated_ecrc), UVM_LOW)
               ...
               if (calculated_ecrc == r_x.ECRC) begin
                    ...
                    foreach (r_x.tlp_q[i])
                         `uvm_info("TX_TL_MONITOR", $sformatf(
                                   "[%s] TX_TLP[%0d] = %08h @ %0t", tag, i, r_x.tlp_q[i], $time),
                                   UVM_LOW)
                    ...
                    if (r_x.compl_status == 3'b001)
                         `uvm_error("TX_TL_MONITOR", $sformatf(
                                    "[%s] UNSUPPORTED_REQ completion received (UR)", tag))
                    if (r_x.compl_status == 3'b100)
                         `uvm_error("TX_TL_MONITOR", $sformatf(
                                    "[%s] COMPLETER_ABORT completion received (CA)", tag))
               ...
               `uvm_info("TL_MON", "Writing completion to analysis port", UVM_HIGH)
          ...
          if (!r_x.ecrc_error && r_x.r_type == 5'b01010) begin
               ...
               `uvm_info("TL_MONITOR", "Static Queue Is Deleted", UVM_HIGH)

               cpl_q.push_back(cpl);

               `uvm_info("TX_MONITOR", $sformatf("Size = %0d", cpl_q.size), UVM_LOW)

               `uvm_info("TX_MON", $sformatf("Completion stored. Queue Size=%0d Tag=%0d",
                                             cpl_q.size(), cpl.tag), UVM_LOW)
     ...
     task ep_collecting_tx_request(Sequence_item r_x);
          ...
          if (r_x.fmt[2]) begin
               `uvm_error(
                   "MALFORMED_TLP",
                   $sformatf(
                       "[%s] Reserved fmt encoding: fmt=%03b (bit2 set) r_type=%05b - Malformed TLP",
          ...
          case (r_x.r_type)
               ...
               default: begin
                    `uvm_error("MALFORMED_TLP", $sformatf(
                               "[%s] Illegal/reserved fmt+r_type combination: fmt=%03b r_type=%05b - Malformed TLP",
                               tag,
                               r_x.fmt,
                               r_x.r_type
                               ))
          ...
          if (r_x.r_type inside {5'b00010, 5'b00100, 5'b00101} && r_x.length != 10'd1) begin
               `uvm_error("MALFORMED_TLP",
                          $sformatf(
                              "[%s] IO/CFG request with Length=%0d (must be 1) - Malformed TLP",
                              tag, r_x.length))
          end
          ...
          if (r_x.e_type inside {MEM_RD, MEM_WR, IO_RD, IO_WR, CFG_RD0, CFG_WR0, CFG_RD1, CFG_WR1}) begin
               ...
               if (!first_be_ok || !last_be_ok) begin
                    `uvm_error(
                        "MALFORMED_TLP",
                        $sformatf(
                            "[%s] Illegal byte-enable pattern: first_BE=%04b last_BE=%04b - Malformed TLP",
          ...
          `uvm_info("RX_TL_MONITOR",
                    $sformatf("[%s] BYTE_COUNT = %0d REQ_ID = %0d TAG = %0d EP_DEVICE = %0h", tag,
                              r_x.byte_count, r_x.req_id, r_x.tag, r_x.ep_device), UVM_LOW)
          `uvm_info("MON", $sformatf("MON: reg_num=%0d ext_reg=%0d", r_x.register_num,
                                     r_x.ext_register_num), UVM_NONE)
          ...
          `uvm_info(
              "TLP_length",
              $sformatf(
                  "[%s] ERR_LENGTH: Expected payload = %0d DW, Captured payload = %0d DW, Missing = %0d DW - Malformed TLP",
          ...
          if (r_x.fmt inside {3'b010, 3'b011}) begin
               ...
               for (int i = 0; i < payload_dw; i++) begin
                    ...
                    if (RX_TL_DL.tl_rx_valid && RX_TL_DL.tl_rx_ready) begin
                         ...
                         `uvm_info("TLP_length", $sformatf(
                                   "[%s] Payload DW #%0d not present - stopping early after %0d/%0d DW captured",
                                   tag,
                                   i,
                                   actual_payload_dw,
                                   payload_dw
          ...
          if (malformed_length) begin
               `uvm_error(
                   "MALFORMED_TLP",
                   $sformatf(
                       "[%s] ERR_LENGTH: Expected payload = %0d DW, Captured payload = %0d DW, Missing = %0d DW - Malformed TLP - PACKET DROPPED, not forwarded to LUT/Scoreboard",
          ...
          if (r_x.td) begin
               ...
               if (calculated_ecrc == r_x.ECRC) begin
                    ...
                    foreach (r_x.tlp_q[i])
                         `uvm_info("RX_TL_MONITOR", $sformatf(
                                   "[%s] RX_TLP[%0d] = %08h @ %0t", tag, i, r_x.tlp_q[i], $time),
                                   UVM_LOW)
                    if (r_x.ep == 1)
                         `uvm_error("RX_TL_MONITOR", $sformatf(
                                    "[%s] POISONED TLP - PACKET DROPPED", tag))
                    else if (malformed_length)
                         `uvm_error("RX_TL_MONITOR", $sformatf(
                                    "[%s] MALFORMED TLP (length mismatch) - PACKET DROPPED", tag))
                    else begin

                         RX_TL_Send.write(r_x);
                         `uvm_info("MON_TO_LUT", $sformatf(
                                   "Sending to LUT: reg=%0d tag=%0d", r_x.register_num, r_x.tag),
                                   UVM_NONE)
     ...
     task ep_sending_rx_completion(Sequence_item t_x);
          ...
          foreach (t_x.tlp_q[i])
               `uvm_info(
                   "RX_TL_MONITOR", $sformatf(
                   "[%s] TL_RX_MONITOR_TLP_PACKET[%0d] = %0h @(%0t)", tag, i, t_x.tlp_q[i], $time),
                   UVM_LOW)
