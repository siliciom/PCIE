class Tag_Manager extends uvm_component;
  ...
  function new(string name="Tag_Manager",
  ...
  function int allocate_tag();
    ...
    for(int i=0;i<$size(tag_busy);i++) begin
      ...
      if(!tag_busy[i]) begin
        ...
        `uvm_info("TAG_MANAGER", $sformatf("TAG_ALLOCATED=%0d OUTSTANDING=%0d", i, outstanding_cnt), UVM_LOW);
    ...
    `uvm_warning("TAG_MGR", "All PCIe tags are outstanding.")
