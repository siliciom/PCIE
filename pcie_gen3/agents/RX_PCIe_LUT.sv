...
class RX_PCIe_LUT extends uvm_component;
     ...
     function void write(Sequence_item req);
          req_q.push_back(req);
          `uvm_info("RX_PCIe_LUT", $sformatf(
                    "LUT WRITE CALLED, queue size after push = %0d, fmt=%03b r_type=%05b",
                    req_q.size(),
                    req.fmt,
                    req.r_type
                    ), UVM_LOW)
     ...
     function void build_phase(uvm_phase phase);
          ...
          if (!uvm_config_db#(PCIe_Cfg_Space_Model)::get(this, "", "cfg_model", cfg_model))
               `uvm_fatal("RX_PCIe_LUT",
                          "Cannot get shared PCIe_Cfg_Space_Model (cfg_model) from Env_Top")

          `uvm_info("RX_PCIe_LUT", $sformatf(
                    "Using shared cfg_model: cfg[0]=%08h cfg[BAR0]=%08h",
                    cfg_model.read_reg(
                        10'h0
                    ),
                    cfg_model.read_reg(
          ...
          bar_cfg_is_io[0] = 0;
          bar_cfg_is_64[0] = 0;
          bar_cfg_mask[0] = 32'hFFF0_0000;
          bar_used[1] = 1;
          bar_cfg_is_io[1] = 0;
          bar_cfg_is_64[1] = 1;
          bar_cfg_mask[1] = 32'hE000_0000;
          bar_used[2] = 0; // BAR2 is the high DW of the BAR1/2 64-bit pair - not independently decoded
          bar_used[3] = 1;
          bar_cfg_is_io[3] = 1;
          bar_cfg_is_64[3] = 0;
          bar_cfg_mask[3] = 32'hFFFF_FF00;
          bar_used[4] = 0;
          bar_used[5] = 0;

          `uvm_info("INIT", $sformatf(
                    "After init: cfg[0]=%08h cfg[BAR0]=%08h",
                    cfg_model.read_reg(
                        10'h0
                    ),
                    cfg_model.read_reg(
     ...
     function automatic bit is_addr_supported(bit [63:0] addr, bit is_io);
          for (int b = 0; b < 6; b++) begin
               ...
               if (!bar_used[b] || bar_cfg_is_io[b] != is_io) continue;
               ...
               end else if (bar_cfg_is_64[b] && (b < 5)) begin
               ...
               if (size == 0) continue;  // BAR not sized/programmed
     ...
     function automatic bit is_ca_trigger_addr(bit [63:0] addr);
          ...
          if (!bar_used[0] || bar_cfg_is_io[0]) return 0;
     ...
     task run_phase(uvm_phase phase);
          ...
          `uvm_info("RX_PCIe_LUT", "LUT RUN PHASE STARTED", UVM_HIGH)
          ...
          forever begin
               ...
               `uvm_info("LUT_REQ", $sformatf(
                         "REQ: fmt=%03b r_type=%05b reg=%0d ext=%0d tag=%0d",
                         req.fmt,
                         req.r_type,
                         req.register_num,
                         req.ext_register_num,
                         req.tag
                         ), UVM_NONE)
               ...
               if (is_mem_wr) begin
                    ...
                    if (!is_addr_supported(req.addr, 0)) begin
                         `uvm_info(
                             "RX_PCIe_LUT",
                             $sformatf(
                                 "MEM_WR to unsupported addr=%0h - UR, posted write dropped (no completion)",
                                 req.addr), UVM_LOW)
                    end else begin
                    ...
                    if (!is_addr_supported(req.addr, 0)) begin
                    ...
                    end  // ERR_COMPLETER_ABORT checker: address IS claimed by BAR0,
                         ...
                         `uvm_info("RX_PCIe_LUT",
                                   $sformatf(
                                       "MEM_RD to addr=%0h hit completer-side failure trap - CA",
                                       req.addr), UVM_LOW)
               ...
               end else if (is_cfg0_wr) begin
                    cfg_write(req);
                    cpl = generate_cfg_wr_cpl(req);
                    cpl_ap.write(cpl);  // CHANGED: was cpl_fifo.put(cpl);
                    tag_mgr.free_tag(cpl.tag);
               end else if (is_cfg1_wr) begin
                    cfg_write(req);
                    cpl = generate_cfg_wr_cpl(req);
                    cpl_ap.write(cpl);  // CHANGED: was cpl_fifo.put(cpl);
                    tag_mgr.free_tag(cpl.tag);
               end else if (is_cfg0_rd) begin
                    if ((req.ep_bus == ep_bus) && (req.ep_device == ep_device) && (req.ep_function_n == ep_function)) begin
                         cpl = generate_cfg_cpl(req);
                         cpl_ap.write(cpl);  // CHANGED: was cpl_fifo.put(cpl);
                         tag_mgr.free_tag(cpl.tag);
                    end else
                         `uvm_info("RX_PCIe_LUT", "Configuration Request not for this EP",
                                   UVM_HIGH);
               end else if (is_cfg1_rd) begin
                    if ((req.ep_bus == ep_bus) && (req.ep_device == ep_device) && (req.ep_function_n == ep_function)) begin
                         cpl = generate_cfg_cpl(req);
                         cpl_ap.write(cpl);  // CHANGED: was cpl_fifo.put(cpl);
                         tag_mgr.free_tag(cpl.tag);
     ...
     task generate_mem_cpl(Sequence_item req);
          ...
          while (bytes_remaining > 0) begin
               ...
               else cpl.lower_addr = {5'b00000, 2'b00};
               ...
               foreach (cpl.payload[i]) cpl.payload[i] = mem_space[current_addr+i*4];
     ...
     function Sequence_item generate_cfg_cpl(Sequence_item req);
          ...
          `uvm_info("RX_PCIE_LUT", $sformatf(
                    "CFG_RD dw_addr=%0h (ext=%0d reg=%0d) data=%08h",
                    dw_addr,
                    req.ext_register_num,
                    req.register_num,
                    cpl.payload[0]
          ...
          `uvm_info(
              "CFG_CPL", $sformatf(
              "[%0t] Generating completion for reg=%0d tag=%0d", $time, req.register_num, req.tag),
              UVM_LOW)
