...
virtual function void build_phase (uvm_phase phase);
		...
		if(!uvm_config_db #(virtual apb_interface)::get (this,"","svif",vif))
		 ...
		 if(!uvm_config_db #(PCIe_Cfg_Space_Model)::get (this,"","cfg_model",cfg_model))   // ← add this
...
task run_phase(uvm_phase phase);
    ...
    forever 
        begin 
          ...
          drive_2_dut(req);
          //`uvm_info("SLV_DRIVER","DRIVER DATA",UVM_HIGH);
          //req.print();
...
task drive_2_dut(apb_slv_xtn xtnh);
    begin
        case(xtnh.op)
            ...
            //slave_err : slv_err(xtnh);   
            //slave_err : slv_error(xtnh);   
        endcase
    end
endtask   
                ...
                begin
                    if(vif.PWRITE == 1'b1)   //write with no wait state
                        begin
                            ...
                            else  vif.PSLVERR <= 1'b1;
                            @(posedge vif.PCLK);
                            vif.PREADY <= 1'b0;
                            vif.PSLVERR <= 1'b0;
                        end
                    else                           //write with no wait state
                        begin
                            vif.PREADY <= 1'b1;
