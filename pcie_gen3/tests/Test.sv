class pcie_base_test extends uvm_test;
     ...
     function void build_phase(uvm_phase phase);
          ...
          DL_Cov   = PCIe_DL_Coverage::type_id::create("DL_Cov", this);
          ...
          for (int unsigned i = 0; i < num_rc; i++) begin
               ...
               uvm_config_db#(PCIe_DL_Coverage)::set(this, rc_name, "DL_Cov", DL_Cov);
          ...
          for (int unsigned i = 0; i < num_ep; i++) begin
               ...
               uvm_config_db#(PCIe_DL_Coverage)::set(this, ep_name, "DL_Cov", DL_Cov);
     ...
     task cfg_read(bit [3:0] ext_reg, bit [5:0] reg_num, output bit [31:0] data);
          ...
          found = 0;
          while (!found) begin
               `uvm_info("TEST", "Waiting for completion...", UVM_HIGH)
               cpl_fifo.get(cpl);
               `uvm_info("TEST", "Completion received", UVM_HIGH)
               `uvm_info("TEST", $sformatf(
                         "REQ TAG = %0d  CPL TAG = %0d  CPL TYPE = %s",
                         rd_seq.req.tag,
                         cpl.tag,
                         cpl.e_type.name()
                         ), UVM_LOW)
     ...
     task do_enumeration();
          ...
          // 1) Vendor/Device ID
          `uvm_info("ENUM", $sformatf("[%0t] Reading Vendor ID", $time), UVM_HIGH)

          cfg_read(4'h0, 6'h0, data);
          `uvm_info("ENUM", $sformatf("[%0t] Vendor ID complete", $time), UVM_HIGH)
          vendor_id      = data[15:0];
          ...
          if (!device_present) begin
               `uvm_fatal("ENUM",
                          "Vendor ID read back as FFFF - no device present, aborting enumeration")
               return;
          end

          `uvm_info("ENUM", $sformatf("[%0t] Reading Header Type", $time), UVM_HIGH)
          cfg_read(4'h0, 6'h3, data);
          `uvm_info("ENUM", $sformatf("[%0t] Header Type complete", $time), UVM_HIGH)
          header_type = data[23:16];

          `uvm_info("ENUM", $sformatf("Reg3 = %08h -> header_type=%02h", data, header_type),
                    UVM_LOW)
          ...
          if (header_type != 8'h00) begin
               `uvm_fatal(
                   "ENUM",
                   $sformatf(
                       "Header type %02h is not a normal (type 0) endpoint - this bench only models type 0",
                       header_type))
               return;
          end
          ...
          for (int bar_num = 0; bar_num < 6; bar_num++) begin
               ...
               `uvm_info("ENUM", $sformatf("BAR%0d original = %08h", bar_num, bar_orig[bar_num]),
                         UVM_LOW)
          end
          ...
          for (int bar_num = 0; bar_num < 6; bar_num++) begin
               ...
               `uvm_info("ENUM", $sformatf(
                         "BAR%0d sizeback=%08h -> is_io=%0b is_64=%0b valid=%0b",
                         bar_num,
                         bar_sizeback[bar_num],
                         bar_is_io[bar_num],
                         bar_is_64[bar_num],
                         bar_valid[bar_num]
                         ), UVM_LOW)
          ...
          `uvm_info("ENUM", "Command register: Mem+IO+BusMaster enabled", UVM_HIGH)
          ...
          for (int bar_num = 0; bar_num < 6; bar_num++) begin
               ...
               `uvm_info("ENUM", $sformatf(
                         "BAR%0d base assigned = %016h (fmt wr=%s rd=%s)",
                         bar_num,
                         bar_base[bar_num],
                         bar_wr_fmt[bar_num].name(),
                         bar_rd_fmt[bar_num].name()
                         ), UVM_LOW)
          end
     ...
     task run_phase(uvm_phase phase);
          ...
          `uvm_info("TEST", "TRIGGERED DONE", UVM_HIGH)
...
class Single_Mem_Wr_Rd_3DW_test extends pcie_base_test;
     ...
     task run_phase(uvm_phase phase);
          ...
          `uvm_info("TEST", $sformatf("BAR0 Base=%h WR_FMT=%s RD_FMT=%s", Mem_Seq_tx.p_addr,
                                      Mem_Seq_tx.p_wr_fmt.name(), Mem_Seq_tx.p_rd_fmt.name()),
                    UVM_NONE)
...
class Multiple_Mem_Wr_Rd_3DW_rand_length_test extends pcie_base_test;
     ...
     task run_phase(uvm_phase phase);
          ...
          repeat (100) begin
               ...
               Seq_tx.p_rd_fmt = bar_rd_fmt[0];
               Seq_tx.p_addr = curr_addr;
               assert (std::randomize(Seq_tx.p_length) with {Seq_tx.p_length inside {[1 : 500]};});
...
class pcie_ral_test extends uvm_test;
     ...
     // builds Env_Top + sets env_cfg + TL_Scb, extend THAT instead ofhat
     ...
     function void build_phase(uvm_phase phase);
          ...
          uvm_config_db#(DL_Scoreboard)::set(this, "*", "DL_Scb", DL_Scb);
          uvm_config_db#(PCIe_TL_Coverage)::set(this, "*", "pcie_cov", pcie_cov);
          uvm_config_db#(PCIe_DL_Coverage)::set(this, "*", "DL_Cov", DL_Cov);

          uvm_config_db#(PCIe_MAC_Scoreboard)::set(this, "*", "mac_scb", mac_scb);
...
class LTSSM_Disabled_test extends pcie_base_test;
     ...
     task run_phase(uvm_phase phase);
          ...
          `uvm_info("TEST", "DISABLED functionality complete -> ending test", UVM_HIGH)
          phase.drop_objection(this);
     endtask : run_phase
...
class LTSSM_Loopback_test extends pcie_base_test;
     ...
     task run_phase(uvm_phase phase);
          ...
          `uvm_info("TEST", "Directing Loopback.Active -> Loopback.Exit on both sides", UVM_HIGH)
          ...
          `uvm_info("TEST", "LOOPBACK_EXIT functionality complete -> ending test", UVM_HIGH)
          phase.drop_objection(this);
     endtask : run_phase
...
class LTSSM_HotReset_test extends pcie_base_test;
     ...
     task run_phase(uvm_phase phase);
          ...
          `uvm_info("TEST", "HOT_RESET functionality complete -> ending test", UVM_HIGH)
          phase.drop_objection(this);
     endtask : run_phase
...
class Single_Mem_Wr_emt_Rd_3DW_test extends pcie_base_test;

     `uvm_component_utils(Single_Mem_Wr_emt_Rd_3DW_test)

     Single_Mem_Wr_emt_Rd_3DW Mem_Seq_tx;

     function new(string name = "Single_Mem_Wr_emt_Rd_3DW_test", uvm_component parent = null);
          super.new(name, parent);
     endfunction
     ...
     task run_phase(uvm_phase phase);
          ...
          Mem_Seq_tx = Single_Mem_Wr_emt_Rd_3DW::type_id::create("Mem_Seq_tx");
          Mem_Seq_tx.p_wr_fmt = bar_wr_fmt[0];
          Mem_Seq_tx.p_rd_fmt = bar_rd_fmt[0];
          Mem_Seq_tx.p_addr = bar_base[0] + 64'h10;
          Mem_Seq_tx.p_length = 1;
          `uvm_info("TEST", $sformatf("BAR0 Base=%h WR_FMT=%s RD_FMT=%s", Mem_Seq_tx.p_addr,
                                      Mem_Seq_tx.p_wr_fmt.name(), Mem_Seq_tx.p_rd_fmt.name()),
                    UVM_NONE)
...
endclass : Single_Mem_Wr_emt_Rd_3DW_test

class Multiple_Mem_Wr_Rd_3DW_tag_outstanding_test extends pcie_base_test;

     `uvm_component_utils(Multiple_Mem_Wr_Rd_3DW_tag_outstanding_test)

     Multiple_Mem_Wr_256_tag Seq_tx;
     bit [5:0] offset;
     bit [63:0] curr_addr;


     function new(string name = "Multiple_Mem_Wr_Rd_3DW_tag_outstanding_test",
     ...
     task run_phase(uvm_phase phase);
          ...
          repeat (250) begin
               ...
               Seq_tx = Multiple_Mem_Wr_256_tag::type_id::create("Seq_tx");
...
endclass : Multiple_Mem_Wr_Rd_3DW_tag_outstanding_test




class Multiple_Mem_Wr_Rd_3DW_tc_vc_test extends pcie_base_test;
     `uvm_component_utils(Multiple_Mem_Wr_Rd_3DW_tc_vc_test)
     Multiple_Mem_Wr_Rd_3DW_tc_vc Seq_tx;
     bit [5:0] offset;
     bit [63:0] curr_addr;
     bit [2:0] tc_perm[8] = '{0, 1, 2, 3, 4, 5, 6, 7};

     function new(string name = "Multiple_Mem_Wr_Rd_3DW_tc_vc_test", uvm_component parent = null);
          super.new(name, parent);
     endfunction
     ...
     task run_phase(uvm_phase phase);
          ...
          tc_perm.shuffle();  // random permutation of 0..7, no repeats

          for (int i = 0; i < 8; i++) begin
               RC_Env[0].PCIe_DLL_Agnt.PCIe_DLL_Drv.cfg.replay_en = 1'b0;
               Seq_tx = Multiple_Mem_Wr_Rd_3DW_tc_vc::type_id::create("Seq_tx");

               Seq_tx.p_wr_fmt = bar_wr_fmt[0];
               Seq_tx.p_rd_fmt = bar_rd_fmt[0];
               Seq_tx.p_addr = curr_addr;
               Seq_tx.p_length = 10;
               Seq_tx.tc = tc_perm[i];  // assign directly, no need to randomize in sequence

               Seq_tx.start(RC_Env[0].PCIe_TL_Agnt.TX_TL_Seqr);
               curr_addr += Seq_tx.p_length * 4;
               `uvm_info("TEST", $sformatf("curr_addr = %0h, tc = %0d", curr_addr, tc_perm[i]),
                         UVM_LOW)
          end
          ...
          phase.drop_objection(this);
     endtask : run_phase
endclass : Multiple_Mem_Wr_Rd_3DW_tc_vc_test
