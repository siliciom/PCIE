...
//     1. injection knob
//        - TL layer:  Error_Inject_Seq.p_inject_err (consumed in
...
//     - declares PASS/FAIL from catcher.hit_cnt (expected-uvm_error
...
//   Checker: pre-existing uvm_error("RX_TL_MONITOR", "ECRC MISMATCH")
//            in agents/PCIe_TL_Monitor.sv (ep_collecting_tx_request).
//---------------------------------------------------------------
class TL_ECRC_Error_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: corrupted ECRC was detected and flagged by RX_TL_MONITOR", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: corrupted ECRC was NOT detected by RX_TL_MONITOR")
...
//---------------------------------------------------------------
// TL_Length_Mismatch_test (Malformed TLP, Fatal class)
//   Inject: ERR_LEN_MISMATCH - header Length lies (post_randomize()
...
class TL_Length_Mismatch_test extends pcie_base_test;
 ...
 // int pass_cnt_before, fail_cnt_before;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: IO read with Length>1 flagged as Malformed", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: IO read with Length>1 was not flagged")
...
class TL_CFG_Length_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: Config read with Length>1 flagged as Malformed", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: Config read with Length>1 was not flagged")
...
//   Checker: uvm_error("MALFORMED_TLP", "Illegal/reserved fmt...")
...
class TL_Fmt_Rtype_Illegal_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    catcher = Error_Report_Catcher::get();
    catcher.reset();
    catcher.arm("MALFORMED_TLP", "Reserved fmt encoding");
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: illegal fmt+r_type combination flagged as Malformed", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: illegal fmt+r_type combination was not flagged")
...
class TL_Bad_BE_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: illegal byte-enable pattern flagged as Malformed", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: illegal byte-enable pattern was not flagged")
...
class TL_EP_Poison_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: poisoned (EP=1) write payload was dropped per completer policy", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: poisoned write was not detected/dropped")
...
class TL_Unsupported_Request_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: read to unsupported address flagged and answered with UR", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: read to unsupported address was not flagged")
...
class TL_Completer_Abort_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: read to CA-trigger address flagged and answered with CA", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: read to CA-trigger address was not flagged")
...
//           TX_DLP_PACKET.
//   Checker: pre-existing uvm_error("TX_DLL_MON", "LCRC MISMATCH")
...
class DLL_LCRC_Error_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: corrupted LCRC detected -> NAK + replay", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: corrupted LCRC was NOT detected")
...
class DLL_DLLP_CRC_Error_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: corrupted DLLP CRC detected (Bad-DLLP path)", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: corrupted DLLP CRC was NOT detected")
...
//            in agents/PCIe_DLL_Monitor.sv (the seq_no > NRS branch).
//---------------------------------------------------------------
class DLL_Seq_Num_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: skipped/out-of-order DL sequence number detected", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: skipped DL sequence number was NOT detected")
...
//            agents/PCIe_DLL_Monitor.sv (rc_monitor_sb/ep_monitor_sb).
//---------------------------------------------------------------
class PHY_STP_Framing_Error_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: corrupted start-of-TLP framing marker detected (real HW: LTSSM Recovery)", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: corrupted framing marker was NOT detected")
...
//            limit") at PCIe_DLL_Driver.sv (replay_num == 3 branch).
//---------------------------------------------------------------
class DLL_Replay_Num_Rollover_test extends pcie_base_test;
  ...
  task run_phase(uvm_phase phase);
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: replay-count rollover (4+ replays of the same packet) detected", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: replay-count rollover was NOT detected")
...
assert(std::randomize(offset) with {
    ...
    if (catcher.hit_cnt > 0)
      `uvm_info("TEST_RESULT", "PASS: withheld Ack past the replay timer limit retriggered replay", UVM_NONE)
    else
      `uvm_error("TEST_RESULT", "FAIL: replay timer did not retrigger a replay when Ack was withheld")
