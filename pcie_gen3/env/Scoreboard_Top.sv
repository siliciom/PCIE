...
class Scoreboard_Top extends uvm_scoreboard;
     ...
     virtual function void write_t(Sequence_item tx_pkt);
          if (is_completion(tx_pkt)) begin
               ...
               `uvm_info("SCOREBOARD8", $sformatf(
                                            "write_t: CPL queued, tx_cpl_q size=%0d e_type=%s",
                                            tx_cpl_q.size(), tx_pkt.e_type.name()), UVM_LOW)
          end else begin
               tx_req_q.push_back(tx_pkt);
               `uvm_info("SCOREBOARD8", $sformatf(
                         "write_t: REQ queued, tx_req_q size=%0d e_type=%s",
                         tx_req_q.size(),
                         tx_pkt.e_type.name()
                         ), UVM_LOW)
          end
     ...
     virtual function void write_r(Sequence_item rx_pkt);
          if (is_completion(rx_pkt)) begin
               ...
               `uvm_info("SCOREBOARD8", $sformatf(
                                            "write_r: CPL queued, rx_cpl_q size=%0d e_type=%s",
                                            rx_cpl_q.size(), rx_pkt.e_type.name()), UVM_LOW)
          end else begin
               rx_req_q.push_back(rx_pkt);
               `uvm_info("SCOREBOARD8", $sformatf(
                         "write_r: REQ queued, rx_req_q size=%0d e_type=%s",
                         rx_req_q.size(),
                         rx_pkt.e_type.name()
                         ), UVM_LOW)
          end
     ...
     task check_requests();
          ...
          forever begin
               ...
               `uvm_info("DEBUG1", "Entered check_requests()", UVM_HIGH)
               tx_pkt = tx_req_q.pop_front();
               rx_pkt = rx_req_q.pop_front();
               `uvm_info("DEBUG123", $sformatf(
                         "TX: req_id=%0h tag=%0h type=%s | RX: req_id=%0h tag=%0h type=%s",
                         tx_pkt.req_id,
                         tx_pkt.tag,
                         tx_pkt.e_type.name(),
                         rx_pkt.req_id,
               ...
               if (rx_pkt.e_type == MEM_WR) begin
                    mem_write(rx_pkt);
               end else if (rx_pkt.e_type == MEM_RD) begin
     ...
     task check_completions();
          ...
          forever begin
               ...
               `uvm_info("DEBUG2", "Entered check_completions()", UVM_HIGH)
               ...
               if (tx_pkt.e_type == CPL_DATA) begin
                    ...
                    if (pending_reads.exists(k)) begin
                         ...
                         `uvm_info("MEM_MODEL_DEBUG", $sformatf(
                                   "CPL_DATA for req_id=%0h tag=%0h has no pending MEM_RD (likely IO_RD/CFG_RD) - skipping mem check",
                                   tx_pkt.req_id,
                                   tx_pkt.tag
                                   ), UVM_LOW)
                    end
     ...
     function automatic void mem_write(Sequence_item pkt);
          ...
          if (pkt.ep) begin
               `uvm_error(
                   "MALFORMED_TLP",
                   $sformatf(
                       "Poisoned MEM_WR (EP=1) received addr=%0h tag=%0h - dropping payload per completer policy",
                       pkt.addr, pkt.tag))
               return;
          end
          ...
          foreach (pkt.payload[i]) begin
               bit [3:0] be;
               if (n == 1) be = pkt.first_BE;
               else if (i == 0) be = pkt.first_BE;
               else if (i == n - 1) be = pkt.last_BE;
               else be = 4'b1111;

               write_dw_with_be(a, pkt.payload[i], be);

               `uvm_info("MEM_MODEL", $sformatf(
                         "MEM WRITE  addr=%0h  data=%08h  be=%0b  stored=%08h",
                         a,
                         pkt.payload[i],
                         be,
                         mem[a]
     ...
     function automatic void mem_register_read(Sequence_item pkt);
          ...
          key = rd_key(pkt);
          `uvm_info("MEM_MODEL_DEBUG", $sformatf(
                    "REGISTERING key=%0h req_id=%0h tag=%0h addr=%0h length=%0d",
                    key,
                    pkt.req_id,
                    pkt.tag,
                    pkt.addr,
          ...
          `uvm_info("MEM_MODEL", $sformatf(
                    "MEM_RD REGISTERED  req_id=%0h tag=%0h addr=%0h length=%0d",
                    pkt.req_id,
                    pkt.tag,
                    pkt.addr,
                    pkt.length
     ...
     function automatic void mem_check_completion(Sequence_item pkt);
          ...
          `uvm_info("MEM_MODEL_DEBUG", $sformatf(
                    "LOOKING UP key=%0h req_id=%0h tag=%0h lower_addr=%0h byte_count=%0d payload_dw=%0d",
                    key,
                    pkt.req_id,
                    pkt.tag,
                    pkt.lower_addr,
          ...
          foreach (pkt.payload[i]) begin
               ...
               if (pkt.payload[i] !== expected) begin
                    `uvm_error(
                        "MEM_MODEL",
                        $sformatf(
                            "MEM READ MISMATCH  addr=%0h  expected=%08h  got=%08h  req_id=%0h tag=%0h",
                            a, expected, pkt.payload[i], pkt.req_id, pkt.tag))
                    frag_pass = 0;
               end else begin
                    `uvm_info("MEM_MODEL_PASS", $sformatf(
                              "MEM READ MATCH     addr=%0h  data=%08h  req_id=%0h tag=%0h",
                              a,
                              pkt.payload[i],
                              pkt.req_id,
                              pkt.tag
          ...
          if (pkt.byte_count == pkt.payload.size() * 4) begin
               if (!rd.any_mismatch && (rd.received_dw == rd.length)) begin
                    ...
                    `uvm_info("MEM_MODEL",
                              $sformatf(
                                  "MEM_RD/CPL_DATA MATCH (mem_pass_cnt=%0d) req_id=%0h tag=%0h",
                                  mem_pass_cnt, pkt.req_id, pkt.tag), UVM_LOW)
               end else begin
                    mem_fail_cnt++;
                    `uvm_error("MEM_MODEL", $sformatf(
                               "MEM_RD/CPL_DATA MISMATCH (mem_fail_cnt=%0d) req_id=%0h tag=%0h — received_dw=%0d expected_length=%0d any_mismatch=%0b",
                               mem_fail_cnt,
                               pkt.req_id,
                               pkt.tag,
                               rd.received_dw,
               ...
               `uvm_info("MEM_MODEL_DEBUG", $sformatf(
                         "Fragment processed (pass=%0b), more expected — req_id=%0h tag=%0h received_dw=%0d/%0d",
                         frag_pass,
                         pkt.req_id,
                         pkt.tag,
                         rd.received_dw,
     ...
     function void compare_tlp(Sequence_item tx_pkt, Sequence_item rx_pkt, string tlp_kind);
          if (tx_pkt.tlp_q.size() !== rx_pkt.tlp_q.size()) begin
               `uvm_error("FAIL", $sformatf("%s TLP SIZE MISMATCH: TX=%0d DWs  RX=%0d DWs",
          ...
          begin
               ...
               foreach (tx_pkt.tlp_q[i]) begin
                    if (tx_pkt.tlp_q[i] !== rx_pkt.tlp_q[i]) begin
                         `uvm_error("FAIL", $sformatf("%s DW[%0d] MISMATCH — TX=%08h  RX=%08h",
                                                      tlp_kind, i, tx_pkt.tlp_q[i],
                                                      rx_pkt.tlp_q[i]))
                         pkt_pass = 0;
                    end
               end
               if (pkt_pass) begin
                    pass_cnt++;
                    `uvm_info("SCOREBOARD", $sformatf("%s TLP MATCH (pass cnt=%0d)", tlp_kind,
                                                      pass_cnt), UVM_LOW)
                    foreach (tx_pkt.tlp_q[i])
                         `uvm_info(
                             "PASS", $sformatf(
                             "  TLP[%0d] TX=%08h  RX=%08h", i, tx_pkt.tlp_q[i], rx_pkt.tlp_q[i]),
                             UVM_LOW)
               end else begin
                    fail_cnt++;
                    `uvm_error("FAIL", $sformatf(
                               "%s TLP MISMATCH (fail cnt=%0d)", tlp_kind, fail_cnt))
               end
          end
     endfunction
     ...
     function void report_phase(uvm_phase phase);
          ...
          `uvm_info("MEM_MODEL", $sformatf(
                    "FINAL MEM SCOREBOARD RESULT: mem_pass_cnt=%0d mem_fail_cnt=%0d",
                    mem_pass_cnt,
                    mem_fail_cnt
                    ), UVM_LOW)
     endfunction
