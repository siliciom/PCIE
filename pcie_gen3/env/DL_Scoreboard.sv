...
class DL_Scoreboard extends uvm_scoreboard;
     ...
     function void write_rc_dllp_tx(Sequence_item pkt);
          ...
          `uvm_info("DL_SCB", $sformatf("RC_DLLP_TX queued (depth=%0d)", rc_dllp_tx_q.size()),
                    UVM_HIGH)
     endfunction

     function void write_ep_dllp_rx(Sequence_item pkt);
          ep_dllp_rx_q.push_back(pkt);
          `uvm_info("DL_SCB", $sformatf("EP_DLLP_RX queued (depth=%0d)", ep_dllp_rx_q.size()),
                    UVM_HIGH)
     endfunction

     function void write_ep_dllp_tx(Sequence_item pkt);
          ep_dllp_tx_q.push_back(pkt);
          `uvm_info("DL_SCB", $sformatf("EP_DLLP_TX queued (depth=%0d)", ep_dllp_tx_q.size()),
                    UVM_HIGH)
     endfunction

     function void write_rc_dllp_rx(Sequence_item pkt);
          rc_dllp_rx_q.push_back(pkt);
          `uvm_info("DL_SCB", $sformatf("RC_DLLP_RX queued (depth=%0d)", rc_dllp_rx_q.size()),
                    UVM_HIGH)
     endfunction
     ...
     function void compare_tlp1(Sequence_item tx_pkt, Sequence_item rx_pkt, string tlp_kind);
          ...
          foreach (tx_pkt.tx_data_sb[i]) begin
               if (tx_pkt.tx_data_sb[i] !== rx_pkt.ep_req_data_sb[i]) begin
                    `uvm_error("FAIL", $sformatf("%s DW[%0d] MISMATCH — TX=%08h  RX=%08h",
                                                 tlp_kind, i, tx_pkt.tx_data_sb[i],
                                                 rx_pkt.ep_req_data_sb[i]))
                    pkt_pass = 0;
               end
          end
          ...
          if (pkt_pass) begin
               DL_pass_cnt++;
               `uvm_info("DL_SCOREBOARD", $sformatf("%s DLP MATCH (pass cnt=%0d)", tlp_kind,
                                                    DL_pass_cnt), UVM_LOW)
               foreach (tx_pkt.tx_data_sb[i])
                    `uvm_info("PASS", $sformatf(
                              "  DLP[%0d] TX=%08h  RX=%08h",
                              i,
                              tx_pkt.tx_data_sb[i],
                              rx_pkt.ep_req_data_sb[i]
                              ), UVM_LOW)
               ...
               DL_fail_cnt++;
               `uvm_error("FAIL", $sformatf("%s DLP MISMATCH (fail cnt=%0d)", tlp_kind, DL_fail_cnt
                          ))
          end
     ...
     function void compare_tlp2(Sequence_item tx_pkt, Sequence_item rx_pkt, string tlp_kind);
          ...
          foreach (tx_pkt.rx_data_sb[i]) begin
               if (tx_pkt.rx_data_sb[i] !== rx_pkt.rc_com_data_sb[i]) begin
                    `uvm_error("FAIL", $sformatf("%s DW[%0d] MISMATCH — TX=%08h  RX=%08h",
                                                 tlp_kind, i, tx_pkt.rx_data_sb[i],
                                                 rx_pkt.rc_com_data_sb[i]))
                    pkt_pass = 0;
               end
          end
     ...
     function void compare_dllp1(Sequence_item tx_pkt, Sequence_item rx_pkt, string tlp_kind);
          ...
          foreach (tx_pkt.rc_dllp_data_sb[i]) begin
               if (tx_pkt.rc_dllp_data_sb[i] !== rx_pkt.rc_dllp_packet_sb[i]) begin
                    `uvm_error("FAIL", $sformatf("%s DW[%0d] MISMATCH — TX=%08h  RX=%08h",
                                                 tlp_kind, i, tx_pkt.rc_dllp_data_sb[i],
                                                 rx_pkt.rc_dllp_packet_sb[i]))
                    pkt_pass = 0;
               end
          end
          ...
          if (pkt_pass) begin
               DLLP_pass_cnt++;
               `uvm_info("DL_SCOREBOARD", $sformatf("%s DLLP MATCH (pass cnt=%0d)", tlp_kind,
                                                    DLLP_pass_cnt), UVM_LOW)
               foreach (tx_pkt.rc_dllp_data_sb[i])
                    `uvm_info("PASS", $sformatf(
                              "  DLLP[%0d] TX=%08h  RX=%08h",
                              i,
                              tx_pkt.rc_dllp_data_sb[i],
                              rx_pkt.rc_dllp_packet_sb[i]
                              ), UVM_LOW)
          end else begin
               DLLP_fail_cnt++;
               `uvm_error("FAIL", $sformatf(
                          "%s DLLP MISMATCH (fail cnt=%0d)", tlp_kind, DLLP_fail_cnt))
          end
     ...
     function void compare_dllp2(Sequence_item tx_pkt, Sequence_item rx_pkt, string tlp_kind);
          ...
          foreach (tx_pkt.ep_dllp_data_sb[i]) begin
               if (tx_pkt.ep_dllp_data_sb[i] !== rx_pkt.ep_dllp_packet_sb[i]) begin
                    `uvm_error("FAIL", $sformatf("%s DW[%0d] MISMATCH — TX=%08h  RX=%08h",
                                                 tlp_kind, i, tx_pkt.ep_dllp_data_sb[i],
                                                 rx_pkt.ep_dllp_packet_sb[i]))
                    pkt_pass = 0;
               end
          end
