class Env_Top extends uvm_env;
     ...
     function void build_phase(uvm_phase phase);
          ...
          if (!uvm_config_db#(env_cfg)::get(this, "", "env_cfg", cfg))
               `uvm_fatal("Env_Top", $sformatf("env_cfg not found for %s", get_full_name()))
          else
               `uvm_info("Env_Top", $sformatf(
                         "%s configured as mode=%s", get_full_name(), cfg.mode.name()), UVM_LOW)

          if (!uvm_config_db#(TL_Scoreboard)::get(this, "", "TL_Scb", TL_Scb))
               `uvm_fatal("Env_Top", $sformatf(
                          "TL_Scoreboard handle not found for %s", get_full_name()))
          if (!uvm_config_db#(DL_Scoreboard)::get(this, "", "DL_Scb", DL_Scb))
               `uvm_fatal("Env_Top", $sformatf(
                          "DL_Scoreboard handle not found for %s", get_full_name()))


          if (!uvm_config_db#(PCIe_TL_Coverage)::get(this, "", "pcie_cov", pcie_cov))
               `uvm_fatal("Env_Top", $sformatf("subscriber handle not found for %s", get_full_name()
                          ))
          if (!uvm_config_db#(PCIe_DL_Coverage)::get(this, "", "DL_Cov", DL_Cov))
               `uvm_fatal("Env_Top", $sformatf("subscriber handle not found for %s", get_full_name()
                          ))
          ...
          if (!uvm_config_db#(PCIe_MAC_Scoreboard)::get(this, "", "mac_scb", mac_scb))
               `uvm_fatal("Env_Top", $sformatf(
                          "PCIe_MAC_Scoreboard handle not found for %s", get_full_name()))
          ...
          if (cfg.mode == EP_MODE) begin
               uvm_config_db#(uvm_active_passive_enum)::set(this, "Apb_Master_Agnt", "is_active",
                                                            UVM_ACTIVE);
               Apb_Master_Agnt = agent_apb_master::type_id::create("Apb_Master_Agnt", this);

               Apb_Slave_Agnt =
                   apb_slv_agent::type_id::create("Apb_Slave_Agnt", this);  // ← add this line
               ...
               Apb_Predictor =
                   uvm_reg_predictor#(seq_item_apb_master)::type_id::create("Apb_Predictor", this);
     ...
     function void connect_phase(uvm_phase phase);
          ...
          if (cfg.mode == RC_MODE) begin
               ...
               PCIe_DLL_Agnt.PCIe_DLL_Mon.rc_RX_MD_ap.connect(DL_Cov.TX_DL_Recvx1);
               ...
               PCIe_DLL_Agnt.PCIe_DLL_Mon.ep_RX_MD_ap.connect(DL_Cov.RX_DL_Recvx1);
