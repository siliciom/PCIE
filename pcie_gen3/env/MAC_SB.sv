class PCIe_MAC_Scoreboard extends uvm_scoreboard;
     ...
     function void write_rc_tx(Sequence_item item);
          rc_tx_cnt++;
          `uvm_info("MAC_SB_RC_TX", $sformatf(
                    "Received RC_TX packet. Queue size(before push)=%0d, Data=%p, Time=%0t",
                    rc_tx_q.size(),
                    item.pma_rc_tx_q,
                    $time
                    ), UVM_LOW)
          ...
          `uvm_info("RC_TX", $sformatf("RC_TX Count=%0d Queue=%0d", rc_tx_cnt, rc_tx_q.size()),
                    UVM_LOW)
     ...
     function void write_ep_rx(Sequence_item item);
          ep_rx_cnt++;
          `uvm_info("MAC_SB_EP_RX", $sformatf(
                    "Received EP_RX packet. Queue size(before push)=%0d, Data=%p, Time=%0t",
                    ep_rx_q.size(),
                    item.pma_ep_rx_q,
                    $time
                    ), UVM_LOW)
          ...
          `uvm_info("EP_RX", $sformatf("EP_RX Count=%0d Queue=%0d", ep_rx_cnt, ep_rx_q.size()),
                    UVM_LOW)
     ...
     function void write_ep_tx(Sequence_item item);
          ep_tx_cnt++;
          `uvm_info("MAC_SB_EP_TX", $sformatf(
                    "Received EP_TX packet. Queue size(before push)=%0d, Data=%p, Time=%0t",
                    ep_tx_q.size(),
                    item.pma_ep_tx_q,
                    $time
                    ), UVM_LOW)
          ...
          `uvm_info("EP_TX", $sformatf("EP_TX Count=%0d Queue=%0d", ep_tx_cnt, ep_tx_q.size()),
                    UVM_LOW)
     ...
     function void write_rc_rx(Sequence_item item);
          rc_rx_cnt++;
          `uvm_info("MAC_SB_RC_RX", $sformatf(
                    "Received RC_RX packet. Queue size(before push)=%0d, Data=%p, Time=%0t",
                    rc_rx_q.size(),
                    item.pma_rc_rx_q,
                    $time
                    ), UVM_LOW)
          ...
          `uvm_info("RC_RX", $sformatf("RC_RX Count=%0d Queue=%0d", rc_rx_cnt, rc_rx_q.size()),
                    UVM_LOW)
     ...
     task check_request1();
          ...
          forever begin
               ...
               `uvm_info("REQ_CHECK", $sformatf(
                         "Before pop: RC_TX_Q=%0d EP_RX_Q=%0d", rc_tx_q.size(), ep_rx_q.size()),
                         UVM_LOW)
               ...
               `uvm_info("REQ_CHECK", $sformatf(
                         "Comparing RC_TX(size=%0d) with EP_RX(size=%0d)",
     ...
     task check_completion1();
          ...
          forever begin
               ...
               `uvm_info("CPL_CHECK", $sformatf(
                         "Before pop: EP_TX_Q=%0d RC_RX_Q=%0d", ep_tx_q.size(), rc_rx_q.size()),
                         UVM_LOW)
               ...
               `uvm_info("CPL_CHECK", $sformatf(
                         "Comparing EP_TX(size=%0d) with RC_RX(size=%0d)",
     ...
     function void compare_tlp1(Sequence_item tx_pkt, Sequence_item rx_pkt, string kind);
          ...
          if (match) begin
               foreach (tx_pkt.pma_rc_tx_q[i]) begin
                    ...
                    `uvm_info(
                        "MAC_SB_COMPARE1", $sformatf(
                        "Index=%0d  TX=%0h  RX=%0h", i, tx_pkt.pma_rc_tx_q[i], rx_pkt.pma_ep_rx_q[i]
                        ), UVM_LOW)
          ...
          if (match) begin
               PL_pass_cnt++;
               `uvm_info("MAC_SB_REQUEST_PASS", $sformatf("[%s] MATCH (pass cnt=%0d) TX=%p RX=%p",
                                                          kind, PL_pass_cnt, tx_pkt.pma_rc_tx_q,
                                                          rx_pkt.pma_ep_rx_q), UVM_LOW)
          end else begin
               PL_fail_cnt++;
               `uvm_error("MAC_SB_FAIL", $sformatf(
                          "[%s] MISMATCH (fail cnt=%0d)\n  TX(size=%0d)=%p\n  RX(size=%0d)=%p",
                          kind,
                          PL_fail_cnt,
                          tx_pkt.pma_rc_tx_q.size(),
                          tx_pkt.pma_rc_tx_q,
                          rx_pkt.pma_ep_rx_q.size(),
                          rx_pkt.pma_ep_rx_q
                          ))
          end
     ...
     function void compare_tlp2(Sequence_item ep_item, Sequence_item rc_item, string kind);
          ...
          if (match) begin
               foreach (ep_item.pma_ep_tx_q[i]) begin
                    ...
                    `uvm_info("MAC_SB_COMPARE2", $sformatf(
                              "Index=%0d  EP_TX=%0h  RC_RX=%0h",
                              i,
          ...
          if (match) begin
               ...
               `uvm_info("MAC_SB_COMPLETION_PASS",
                         $sformatf("[%s] MATCH (pass cnt=%0d) EP_TX=%p RC_RX=%p", kind,
                                   PL_pass_cnt, ep_item.pma_ep_tx_q, rc_item.pma_rc_rx_q), UVM_LOW)
          end else begin
               PL_fail_cnt++;
               `uvm_error("MAC_SB_FAIL", $sformatf(
                          "[%s] MISMATCH (fail cnt=%0d)\n  EP_TX(size=%0d)=%p\n  RC_RX(size=%0d)=%p",
                          kind,
                          PL_fail_cnt,
                          ep_item.pma_ep_tx_q.size(),
                          ep_item.pma_ep_tx_q,
                          rc_item.pma_rc_rx_q.size(),
                          rc_item.pma_rc_rx_q
                          ))
          end
     ...
     function void report_phase(uvm_phase phase);
          `uvm_info("SB12", $sformatf(
                    "PCIe_MAC_Scoreboard FINAL RESULT: pass=%0d fail=%0d", PL_pass_cnt, PL_fail_cnt
                    ), UVM_LOW)
