...
class FC_Manager extends uvm_component;
     ...
     function void consume_credit(vc_id_e vc, packet_type_e pkt_type, int header_credit,
                                  int data_credit);

          `uvm_info("FC_Manager", $sformatf("[%0t] consume_credit called (vc=%0d)", $time, vc),
                    UVM_LOW);

          `uvm_info(
              "FC_Manager", $sformatf(
              "vc=%0d pkt_type=%s hc=%0d dc=%0d", vc, pkt_type.name(), header_credit, data_credit),
              UVM_LOW);
          ...
          $display("vc=%0d pkt_type=%s hc=%0d dc=%0d", vc, pkt_type.name(), header_credit,
                   data_credit);
     ...
     function void calc_required_credit(Sequence_item req, output int hdr_credit,
                                        output int data_credit);

          case (req.pkt_type)

               P: begin
                    data_credit = (req.length + 3) / 4;
                    hdr_credit  = 1;
               end
               NP:
               if (req.e_type == MEM_RD) begin
                    data_credit = 0;
                    hdr_credit  = 1;
               end else begin
                    data_credit = (req.length + 3) / 4;
     ...
     function void return_credit(vc_id_e vc, packet_type_e pkt_type, int header_credit,
                                 int data_credit);
          ...
          $display("RETURN CREDIT: VC=%0d BEFORE_PH=%0d ADD_PH=%0d", vc, ph_avail[vc],
                   header_credit);
     ...
     function void send_fc_update(vc_id_e vc);
          ...
          $display("VC%0d: PH=%0d PD=%0d NPH=%0d NPD=%0d CPLH=%0d CPLD=%0d", vc, ph_avail[vc],
                   pd_avail[vc], nph_avail[vc], npd_avail[vc], cplh_avail[vc], cpld_avail[vc]);
     ...
     function bit has_sufficient_credit(vc_id_e vc, Sequence_item req);
          ...
          if (hdr_short || data_short) begin
               ...
               `uvm_info(
                   "FC",
                   $sformatf(
                       "\nInsufficient Flow Control Credits on VC%0d\n\Packet Type : %s\n\Header Credits : Required=%0d Available=%0d Need=%0d more\n\Data Credits   : Required=%0d Available=%0d Need=%0d more",
                       vc, req.pkt_type.name(), hdr_req, hdr_avail, hdr_short, data_req,
                       data_avail, data_short), UVM_HIGH)
